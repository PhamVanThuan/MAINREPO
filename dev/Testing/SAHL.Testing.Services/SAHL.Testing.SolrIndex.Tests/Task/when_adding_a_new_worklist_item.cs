...
{
    public class when_adding_a_new_worklist_item : SolrIndexTest
    {
        [Test]
        public void it_should_update_username_for_the_instance_in_the_task_index()
        {
            ...
            var setupTaskQueryResult = setupTaskQuery.Result.Results.Skip(1).FirstOrDefault();
            ...
            long instanceId = setupTaskQueryResult.InstanceId;
            Guid correlationId = Guid.NewGuid();
            string activityName = ThirdPartyInvoices.Activities.AcceptInvoice;
            string userName = "SAHL\\InvoiceProcessor";
            int userOrganisationStructureKey = TestApiClient.Get<HaloUsersDataModel>(new { ADUserName = "InvoiceProcessor" })
                .Where(x => x.OrgStructureDescription.Contains("Loss Control - Operations - Consultant")).FirstOrDefault().UserOrganisationStructureKey;
            metadata.Add(SAHL.Core.Services.ServiceRequestMetadata.HEADER_USERNAME, userName);
            metadata.Add(SAHL.Core.Services.ServiceRequestMetadata.HEADER_USERORGANISATIONSTRUCTUREKEY, userOrganisationStructureKey.ToString());
            ...
            searchFilters.Clear();
            searchFilters.Add(new SearchFilter("State", ThirdPartyInvoices.States.AcceptForProcessing));
            var assertionTaskQueryResults = SearchTaskIndexByInstanceId(instanceId, ThirdPartyInvoices.States.AcceptForProcessing, searchFilters);
