...
IF OBJECT_ID('X2.tu_State') is null
BEGIN 
	DECLARE @Qry VARCHAR(1024)
	SET @Qry =	'CREATE TRIGGER X2.tu_State ON  X2.State ' +
				'FOR UPDATE ' +
				'AS ' +
				'BEGIN ' +
				'SET NOCOUNT ON; ' +
...
ALTER TRIGGER X2.tu_State ON  X2.State
  FOR UPDATE
...
BEGIN	
	...
	FROM		INSERTED i  			
	INNER JOIN	x2.X2.Instance ins (NOLOCK)  ON ins.StateID = i.ID
