...
IF OBJECT_ID('X2.tu_Instance') is null
BEGIN 
	DECLARE @Qry VARCHAR(1024)
	SET @Qry =	'CREATE TRIGGER X2.tu_Instance ON  X2.Instance ' +
				'FOR UPDATE ' +
				'AS ' +
				'BEGIN ' +
				'SET NOCOUNT ON; ' +
...
ALTER TRIGGER X2.tu_Instance ON  X2.Instance
  FOR UPDATE
...
BEGIN	
	...
	SELECT		1,CONVERT(XML, CONVERT(NVARCHAR(MAX), i.ID)),18 -- SELECT * FROM [2am].dbo.GenericKeyType
	FROM		INSERTED i  		
