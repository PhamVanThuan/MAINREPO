...
namespace SAHL.Core.Specs.LoggingSpecs
{
    public class when_logging_onexitmethod : WithFakes
    {
        ...
        private Because of = () =>
        {
            databaseLogger.LogOnExitMethod(logLevel, source, application, user, method, parameters);
        };

        private It should_call_the_corresponding_rawlogger_method = () =>
        {
            databaseLoggerDataManager.WasToldTo(x => x.StoreLog(Logger.MethodExit, source, application, user, method, string.Empty, parameters));
        };
    }
}