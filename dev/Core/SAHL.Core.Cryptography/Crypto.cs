...
// https://github.com/brockallen/BrockAllen.MembershipReboot/blob/master/util/PasswordStretchingTimings/PasswordStretchingTimings/System.Web.Helpers.Crypto.cs
...
namespace SAHL.Core.Cryptography
{
    public class Crypto
    {
        ...
        public static string Hash(byte[] input, string algorithm = "sha256")
        {
            ...
            var hashData = HashToBytes(input, algorithm);
        ...
        public static string DecryptRSAWithPrivateKey(byte[] dataToDecrypt, string privateKey)
        {
            RSACryptoServiceProvider rsaEncrypt = new RSACryptoServiceProvider();
            rsaEncrypt.FromXmlString(privateKey);
            var cipherText = rsaEncrypt.Decrypt(dataToDecrypt, false);
