...
{
    public class when_get_all_failed_and_pending : WithFakes
    {
        ...
        private Establish context = () =>
        {
            ...
            dbFactory.FakedDb.InEventContext()
                .WhenToldTo(x => x.Select(Arg.Any<GetCommandsThatHaveFailedStatement>())).Return(new List<CommandKeyModel> { new CommandKeyModel { CommandKey = 1 } });
            dbFactory.FakedDb.InEventContext().WhenToldTo(x => x.SelectOneWhere<CommandDataModel>(Arg.Any<string>(), Arg.Any<object>())).Return(commandModel);
        ...
        private Because of = () =>
        {
            CommandSessionFactory.GetAllFailedAndPending(DeserialiseFunc);
        };
