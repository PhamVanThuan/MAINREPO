...
{
    public class when_accessing_latency_for_last_minute : WithFakes
    {
        ...
        private Because of = () =>
        {
            metrics = commandServiceRequestMetrics.CommandLatencyForLastMinute;
        };

        private It should_contain_only_the_daily_metric = () =>
        {
            metrics.First().Key.ShouldEqual(FakeMetricsFactory.minuteMetricDescription);
        };
