...
{
    public class when_unlocking_a_user_account_that_exists : WithFakes
    {
        ...
        private Because of = () =>
        {
            userManager.UnlockUserAccount(userId);
        };

        private It should_deactivate_the_user = () =>
        {
            userDataManager.WasToldTo(x => x.UnlockUserAccount(userId));
        };
    }
}