...
{
    public class when_logging_onentermethod_and_rawlog_throws : WithFakes
    {
        ...
        private Establish context = () =>
            {
                ...
                rawLogger.WhenToldTo(x => x.LogOnEnterMethod(loggerSource.LogLevel, loggerSource.Name, loggerAppSource.ApplicationName, user, method, parameters))
                    .Throw(new Exception());
        ...
        private Because of = () =>
        {
            exception = Catch.Exception(() => logger.LogOnEnterMethod(loggerSource, user, method, parameters));
        };

        private It should_call_the_corresponding_rawlogger_method = () =>
        {
            rawLogger.WasToldTo(x => x.LogOnEnterMethod(loggerSource.LogLevel, loggerSource.Name, loggerAppSource.ApplicationName, user, method, parameters));
        };
