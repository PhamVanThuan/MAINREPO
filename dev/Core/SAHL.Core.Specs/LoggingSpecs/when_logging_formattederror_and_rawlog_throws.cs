...
{
    public class when_logging_formattederror_and_rawlog_throws : WithFakes
    {
        ...
        private Establish context = () =>
            {
                ...
                rawLogger.WhenToldTo(x => x.LogFormattedError(loggerSource.LogLevel, loggerSource.Name, loggerAppSource.ApplicationName, user, method, formattedmessage, parameters, formattingArgs))
                    .Throw(new Exception());
        ...
        private Because of = () =>
        {
            exception = Catch.Exception(() => logger.LogFormattedError(loggerSource, user, method, formattedmessage, parameters, formattingArgs));
        };

        private It should_call_the_corresponding_rawlogger_method = () =>
        {
            rawLogger.WasToldTo(x => x.LogFormattedError(loggerSource.LogLevel, loggerSource.Name, loggerAppSource.ApplicationName, user, method, formattedmessage, parameters, formattingArgs));
        };
