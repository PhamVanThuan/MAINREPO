...
{
    public class when_logging_formatteddebug_no_parameters_and_rawlog_throws : WithFakes
    {
        ...
        private Establish context = () =>
            {
                ...
                rawLogger.WhenToldTo(x => x.LogFormattedDebug(loggerSource.LogLevel, loggerSource.Name, loggerAppSource.ApplicationName, user, method, formattedmessage, formattingArgs))
                    .Throw(new Exception());
        ...
        private Because of = () =>
        {
            exception = Catch.Exception(() => logger.LogFormattedDebug(loggerSource, user, method, formattedmessage, formattingArgs));
        };

        private It should_call_the_corresponding_rawlogger_method = () =>
        {
            rawLogger.WasToldTo(x => x.LogFormattedDebug(loggerSource.LogLevel, loggerSource.Name, loggerAppSource.ApplicationName, user, method, formattedmessage, formattingArgs));
        };
