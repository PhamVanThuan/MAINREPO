...
{
    public class when_logging_warning_and_rawlog_throws : WithFakes
    {
        ...
        private Establish context = () =>
            {
                ...
                rawLogger.WhenToldTo(x => x.LogWarning(loggerSource.LogLevel, loggerSource.Name, loggerAppSource.ApplicationName, user, method, message, parameters))
                    .Throw(new Exception());
        ...
        private Because of = () =>
        {
            exception = Catch.Exception(() => logger.LogWarning(loggerSource, user, method, message, parameters));
        };

        private It should_call_the_corresponding_rawlogger_method = () =>
        {
            rawLogger.WasToldTo(x => x.LogWarning(loggerSource.LogLevel, loggerSource.Name, loggerAppSource.ApplicationName, user, method, message, parameters));
        };
