...
{
    public class when_logging_formattedwarning_and_rawlog_throws : WithFakes
    {
        ...
        private Establish context = () =>
            {
                ...
                rawLogger
                    .WhenToldTo(x => x.LogFormattedWarning(loggerSource.LogLevel, loggerSource.Name, loggerAppSource.ApplicationName, user, method, formattedmessage, parameters, formattingArgs))
                    .Throw(new Exception());
        ...
        private Because of = () =>
        {
            exception = Catch.Exception(() => logger.LogFormattedWarning(loggerSource, user, method, formattedmessage, parameters, formattingArgs));
        };

        private It should_call_the_rawlogger_LogOnEnter_method = () =>
        {
            rawLogger.WasToldTo(x => x.LogFormattedWarning(loggerSource.LogLevel, loggerSource.Name, loggerAppSource.ApplicationName, user, method, formattedmessage, parameters, formattingArgs));
        };
