...
{
    public class when_logging_methodmetric_and_action_throws_no_parameters : WithFakes
    {
        ...
        private Because of = () =>
        {
            exception = Catch.Exception(() => logger.LogMethodMetric(loggerSource, user, method, () => { throw new Exception("Test Exception"); }));
        };

        private It should_not_call_the_corresponding_rawlogger_method = () =>
        {
            rawMetricsLogger.WasNotToldTo(x => x.LogLatencyMetric(loggerSource.Name, loggerAppSource.ApplicationName, user, method, startTime, duration));
            rawMetricsLogger.WasNotToldTo(x => x.LogThroughputMetric(loggerSource.Name, loggerAppSource.ApplicationName, user, method));
        };
