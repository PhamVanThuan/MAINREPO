...
{
    public class when_adding_a_value_to_an_existing_non_leaf_node : WithFakes
    {
        Establish that = () =>
        {
            keyDelimiter = ".";
            tree = new DelimitedRadixTree<int>(keyDelimiter);
            firstKey = "process.workflow.state";
            firstValue = 1;
        ...
        {
            NodeHelpers.GetNodeByKey(tree, "state").NodeValue.ShouldEqual(firstValue);
            NodeHelpers.GetNodeByKey(tree, "workflow").NodeValue.ShouldEqual(secondValue);
            NodeHelpers.GetNodeByKey(tree, "process").NodeValue.ShouldEqual(thirdValue);
        };
