...
{
    public class when_adding_a_duplicate_value_exact : WithFakes
    {
        private Establish that = () =>
        {
            keyDelimiter = ".";
            tree = new DelimitedRadixTree<int>(keyDelimiter, 0, StringComparer.OrdinalIgnoreCase);
            firstKey = "process";
            firstValue = 1;
            secondKey = "PROCESS";
            secondValue = 2;
        ...
        private Because of = () =>
        {
            exception = Catch.Exception(() => tree.Add(secondKey, secondValue));
        };
