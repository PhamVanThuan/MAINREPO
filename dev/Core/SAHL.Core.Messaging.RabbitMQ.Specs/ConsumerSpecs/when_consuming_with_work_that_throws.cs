...
{
    public class when_consuming_with_work_that_throws : WithFakes
    {
        ...
        Establish context = () =>
        {
            ...
            workAction = (message) => { wasWorkPerformed = true; throw new Exception(); };
            isCancelled = () => 
            {
                timesInLoop++;
                if (timesInLoop > 1)
        ...
        It should_nack_the_message = () =>
        {
            model.WasToldTo(x => x.BasicNack(Param.IsAny<ulong>(), Param.IsAny<bool>(), true));
        };
    }
}
