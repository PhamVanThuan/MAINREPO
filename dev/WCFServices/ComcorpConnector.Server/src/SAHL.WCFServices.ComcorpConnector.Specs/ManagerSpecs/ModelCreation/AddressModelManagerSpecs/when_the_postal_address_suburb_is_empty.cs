...
{
    public class when_the_postal_address_suburb_is_empty : WithCoreFakes
    {
        ...
        private Establish context = () =>
        {
            ...
            propertyAddress = new ResidentialAddressModel("2", "Oakland Avenue", string.Empty, string.Empty, string.Empty, "Hillcrest", "Hillcrest", "Kwazulu-Natal", "3610", "South Africa",
                true);
            applicant.PostalAddressSuburb = string.Empty;
        };

        private Because of = () =>
        {
            applicantAddresses = modelManager.PopulateAddresses(applicant, propertyAddress, null);
            applicantPostalAddress = applicantAddresses.Where(x => x.AddressType == AddressType.Postal && x.AddressFormat == AddressFormat.FreeText)
                .FirstOrDefault();
        };

        private It should_not_add_the_postal_address = () =>
        {
            applicantPostalAddress.ShouldBeNull();
        };

        private It should_still_add_the_residential_address = () =>
        {
            applicantAddresses.Where(x => x.AddressType == AddressType.Residential && x.AddressFormat == AddressFormat.FreeText)
            .FirstOrDefault()
            .ShouldNotBeNull();
        };
