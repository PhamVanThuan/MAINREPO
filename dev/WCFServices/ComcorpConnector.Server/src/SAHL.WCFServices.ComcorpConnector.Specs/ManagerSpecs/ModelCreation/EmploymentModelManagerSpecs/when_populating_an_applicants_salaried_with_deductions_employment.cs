...
{
    public class when_populating_an_applicants_salaried_with_deductions_employment : WithCoreFakes
    {
        private static EmploymentModelManager modelManager;
        private static ValidationUtils _validationUtils;
        private static SalaryDeductionEmploymentModel employmentModel;
        ...
        private Establish context = () =>
        {
            ...
            applicant.CurrentEmploymentType = "Salaried with Deduction";
        ...
        private Because of = () =>
        {
            employmentModel = modelManager.PopulateEmployment(applicant).First() as SalaryDeductionEmploymentModel;
        };

        private It should_map_the_employment_type = () =>
        {
            employmentModel.EmploymentType.ShouldEqual(EmploymentType.SalariedwithDeduction);
        };
        ...
        {
            employmentModel.SalaryDeductionRemunerationType.ShouldEqual(SalaryDeductionRemunerationType.Salaried);
        };
        ...
        {
            employmentModel.EmployeeDeductions.Where(x => x.Type == EmployeeDeductionTypeEnum.MedicalAid && x.Value > 0).FirstOrDefault().ShouldNotBeNull();
        };

        private It should_populate_the_employee_paye_deduction = () =>
        {
            employmentModel.EmployeeDeductions.Where(x => x.Type == EmployeeDeductionTypeEnum.PayAsYouEarnTax && x.Value > 0).FirstOrDefault().ShouldNotBeNull();
        };

        private It should_populate_the_PensionOrProvidendFund_deduction = () =>
        {
            employmentModel.EmployeeDeductions.Where(x => x.Type == EmployeeDeductionTypeEnum.PensionOrProvidendFund && x.Value > 0).FirstOrDefault().ShouldNotBeNull();
        };

        private It should_populate_the_employee_UnemploymentInsurance_deduction = () =>
        {
            employmentModel.EmployeeDeductions.Where(x => x.Type == EmployeeDeductionTypeEnum.UnemploymentInsurance && x.Value > 0).FirstOrDefault().ShouldNotBeNull();
        };
    }
}