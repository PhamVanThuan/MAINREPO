    ...
    public class DataAnnotationsModelValidatorProvider : AssociatedValidatorProvider
    {
        ...
        protected override IEnumerable<ModelValidator> GetValidators(ModelMetadata metadata, IEnumerable<Attribute> attributes)
        {
            ...
            try
            {
                ...
                if (AddImplicitRequiredAttributeForValueTypes /*&& metadata.IsRequired */ && !propertyAttributes.Any(a => a is RequiredAttribute))
                {
                    propertyAttributes = propertyAttributes.Concat(new[] { new RequiredAttribute() });
                }

                // Produce a validator for each validation attribute we find
                foreach (ValidationAttribute attribute in propertyAttributes.OfType<ValidationAttribute>())
                {
