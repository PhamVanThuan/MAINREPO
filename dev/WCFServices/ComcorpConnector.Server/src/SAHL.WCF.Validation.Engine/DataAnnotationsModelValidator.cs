    ...
    public class DataAnnotationsModelValidator : ModelValidator
    {
        ...
        protected virtual ModelValidationResult GetModelValidationResult(ValidationResult result)
        {
            return new ModelValidationResult
            {
                ...
                MemberName = Metadata.FullName + (string.IsNullOrEmpty(Metadata.PropertyName) && result.MemberNames != null 
                                    && result.MemberNames.Count() > 0 ? "." + result.MemberNames.FirstOrDefault() : string.Empty)
            };
        }
    }
}
