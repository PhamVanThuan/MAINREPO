describe("Client Capture Specifications -> ", function(){
	...
	beforeEach(function() {
		...
		switchPage = applicationPreCheckPage.proceedForSwitchApplication();
		switchResultsPage = switchPage.fillForm();
		clientCapturePage = switchResultsPage.apply();
	});

	describe("when submitting the application and the first applicant has indicated they are married in community of property-> ", function() {
		var async = new AsyncSpec(this);
		...
		it("should change the description of the button to read Next Applicant", function() {
			...
			var answers = { isIncomeContributor: "Yes", isHappyWeDoITC: "Yes", isSalaryPaidToCapitec: "Yes", isMarriedInCOP: "Yes" };
