describe("Multiple Applicants With ITC Older Than 30 Days Spec ->", function(){
			...
			capitec.transaction(function(err){
				query(capitec.queries.getPersonDetailsOfApplicantsOnPortalDeclinedApplication.toString(), [], function(err, recordset){
					...
					query(capitec.queries.updatePersonITCDate.toString(), 
						[{name: "IDNumber", value: FirstAvailableIDNumber}, {name: "DaysFromToday", value: "-32"}], 
						function(err, recordset){
	...
	describe("when submitting an application with applicants that have an existing itc older than 30 days->", function(){
		...
		it("should request a new ITC", function(){
			browser.sleep(5000);
			capitec.transaction(function(err){
				query(capitec.queries.getPersonITCDetailsByIDNumber.toString(), 
					[{name: "IDNumber", value: FirstAvailableIDNumber}], 
