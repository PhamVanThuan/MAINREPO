...
{
    public class when_reversing_payment_process_for_an_invoice : WithFakes
    {
        ...
        private Establish context = () =>
        {
            ...
            financeDomainServiceClient.WhenToldTo(x => x.PerformCommand(Param<ReturnThirdPartyInvoiceToPaymentQueueCommand>.Matches(y =>
        ...
        private Because of = () =>
        {
            result = workflowProcess.ReturnInvoiceToPaymentQueue(messages, thirdPartyInvoiceKey, metadata);
        };

        private It should_approve_third_party_invoice = () =>
        {
            financeDomainServiceClient.WasToldTo(x => x.PerformCommand(Param<ReturnThirdPartyInvoiceToPaymentQueueCommand>.Matches(y =>
                y.ThirdPartyInvoiceKey == thirdPartyInvoiceKey
                ), metadata));
        };
