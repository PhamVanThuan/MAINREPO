...
/**************************************************
 Statement  : COMMON\CBOGetAffordabilityAssessmentConfirmedByAppKey
 Change Date: 2015/06/26 10:33:58 AM - Version 1
 Change User: SAHL\craigf
 **************************************************/
if( not exists(select StatementName from [2am].dbo.uiStatement where StatementName = 'CBOGetAffordabilityAssessmentConfirmedByAppKey' and ApplicationName = 'COMMON' and Version = 1) ) 
begin
   declare @higherversion int;
   -- check version numbers to determine if we can do insert
   select @higherversion = count(*) from [2am].dbo.uiStatement where StatementName = 'CBOGetAffordabilityAssessmentConfirmedByAppKey' and ApplicationName = 'COMMON' and Version > 1
   ...
   begin
       ...
       begin
           if( exists(select statementname from [2am].dbo.uiStatement where statementname = 'CBOGetAffordabilityAssessmentConfirmedByAppKey' and ApplicationName = 'COMMON' ) )
	            delete from [2am].dbo.uiStatement where statementname = 'CBOGetAffordabilityAssessmentConfirmedByAppKey' and ApplicationName = 'COMMON'; 
       end
       ...
       values ('COMMON', 'CBOGetAffordabilityAssessmentConfirmedByAppKey', GetDate(), 1, 'SAHL\craigf',
                'select 
...
and 
	ass.AffordabilityAssessmentStatusKey = 2 -- Confirmed',
                1, GetDate());
   end
end
