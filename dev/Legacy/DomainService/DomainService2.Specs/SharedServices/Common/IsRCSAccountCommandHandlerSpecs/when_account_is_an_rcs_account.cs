...
namespace DomainService2.Specs.SharedServices.Common.IsRCSAccountCommandHandlerSpecs
{
	...
	public class when_account_is_an_rcs_account : DomainServiceSpec<IsRCSAccountCommand, IsRCSAccountCommandHandler>
	{
		...
		Establish context = () =>
		{
			...
			originationSource.WhenToldTo(x => x.Key).Return((int)OriginationSources.RCS);
		...
		It result_should_be_true = () =>
		{
			isRCSAccount.ShouldBeTrue();
		};
	}
}
