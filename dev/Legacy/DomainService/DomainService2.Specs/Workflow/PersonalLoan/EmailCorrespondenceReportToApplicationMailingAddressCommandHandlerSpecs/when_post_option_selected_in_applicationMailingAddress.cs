...
namespace DomainService2.Specs.Workflow.PersonalLoan.EmailCorrespondenceReportToApplicationMailingAddressCommandHandlerSpecs
{
    ...
    public class when_post_option_selected_in_applicationMailingAddress : DomainServiceSpec<EmailCorrespondenceReportToApplicationMailingAddressCommand,EmailCorrespondenceReportToApplicationMailingAddressCommandHandler>
    {
        ...
        Establish context = () =>
            {
                ...
                cm.WhenToldTo(x => x.Key).Return((int)SAHL.Common.Globals.CorrespondenceMediums.Post);
        ...
        It should_send_the_correspondence_report = () =>
            {
                correspondenceRepository.WasNotToldTo(x => x.SendCorrespondenceReportToLegalEntity(Param<ISendCorrespondenceRequest>.IsAnything));
            };
    }
}
