...
namespace DomainService2.Specs.Workflow.DebtCounselling.SendLitigationReminderInternalEmailCommandHandlerSpecs
{
    ...
    public class When_send_litigation_reminder_email_company : WithFakes
    {
        ...
        static ILegalEntityCompany client;
        ...
        Establish context = () =>
        {
            ...
            client.WhenToldTo(x => x.RegistrationNumber).Return("1235");
            client.WhenToldTo(x => x.GetLegalName(LegalNameFormat.Full)).Return("Mr Company Client");
            legalEntityType.WhenToldTo(x => x.Key).Return((int)SAHL.Common.Globals.LegalEntityTypes.Company);
