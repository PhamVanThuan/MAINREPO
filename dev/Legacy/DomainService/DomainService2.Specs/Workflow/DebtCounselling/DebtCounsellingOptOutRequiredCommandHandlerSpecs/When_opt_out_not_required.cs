...
namespace DomainService2.Specs.Workflow.DebtCounselling.DebtCounsellingOptOutRequiredCommandHandlerSpecs
{
    ...
    public class When_opt_out_not_required : WithFakes
    {
        ...
        Establish context = () =>
        {
            ...
            account.WhenToldTo(x => x.Product.Key).Return((int)SAHL.Common.Globals.Products.VariableLoan); // variable loan

            account.WhenToldTo(x => x.InstallmentSummary).Return(accountInstallmentSummary);
            accountInstallmentSummary.WhenToldTo(x => x.IsInterestOnly).Return(false); // not interest only
            ...
            financialAdjustment.WhenToldTo(x => x.FinancialAdjustmentStatus.Key).Return((int)FinancialAdjustmentStatuses.Active);
            financialAdjustment.WhenToldTo(x => x.FinancialAdjustmentSource).Return(financialAdjustmentSource);
            financialAdjustmentSource.WhenToldTo(x => x.Key).Return((int)FinancialAdjustmentSources.Origination); // not staff of defending cancellations
