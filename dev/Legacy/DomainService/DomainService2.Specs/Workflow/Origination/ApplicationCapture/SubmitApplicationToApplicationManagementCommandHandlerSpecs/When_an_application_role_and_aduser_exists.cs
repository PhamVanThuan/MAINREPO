...
namespace DomainService2.Specs.Workflow.Origination.ApplicationCapture.SubmitApplicationToApplicationManagementCommandHandlerSpecs
{
    ...
    public class When_an_application_role_and_aduser_exists : DomainServiceSpec<SubmitApplicationToApplicationManagementCommand, SubmitApplicationToApplicationManagementCommandHandler>
    {
        ...
        It should_create_external_activity = () =>
        {
            x2Repository.WasToldTo(x => x.CreateAndSaveActiveExternalActivity(Param.IsAny<string>(), Param.IsAny<long>(), Param.IsAny<string>(), Param.IsAny<string>(), Param.IsAny<string>()));
        };
    }
}