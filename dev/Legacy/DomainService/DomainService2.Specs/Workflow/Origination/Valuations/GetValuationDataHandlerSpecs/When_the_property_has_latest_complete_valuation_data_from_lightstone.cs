...
namespace DomainService2.Specs.Workflow.Origination.Valuations.GetValuationData
{
    ...
    public class When_the_property_has_latest_complete_valuation_data_from_lightstone : GetValuationDataCommandHandlerBase
    {
        Establish context = () =>
        {
            messages = new DomainMessageCollection();

            IProperty property = BuildPropertyWithLatestValuation((int)DataProviders.LightStone);
        ...
        It should_return_an_lightstone_property_id = () =>
        {
            ((string)command.ValuationDataResult["LightstonePropertyID"]).ShouldEqual(TEST_PROPERTY_ID);
        };
    }
}