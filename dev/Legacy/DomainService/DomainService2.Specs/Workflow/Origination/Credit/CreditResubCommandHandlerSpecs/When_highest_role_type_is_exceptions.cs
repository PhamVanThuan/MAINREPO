...
namespace DomainService2.Specs.Workflow.Origination.Credit.CreditResubCommandHandlerSpecs
{
    ...
    public class When_highest_role_type_is_exceptions : WithFakes
    {
        ...
        protected static IApplicationRole exceptionsRole;
        protected static IApplicationRoleType exceptionsRoleType;
        ...
        Establish context = () =>
        {
            exceptionsRole = An<IApplicationRole>();
            exceptionsRoleType = An<IApplicationRoleType>();
            ...
            exceptionsRoleType.WhenToldTo(x => x.Key).Return((int)OfferRoleTypes.CreditExceptionsD);
            exceptionsRole.WhenToldTo(x => x.ApplicationRoleType).Return(exceptionsRoleType);
            exceptionsRole.WhenToldTo(x => x.LegalEntityKey).Return(2);
            ...
            managerRole.WhenToldTo(x => x.LegalEntityKey).Return(1);
            ...
            applicationRoles.Add(messages, exceptionsRole);
        ...
        It should_create_an_exceptions_role = () =>
        {
            applicationRepo.WasToldTo(x => x.CreateAndSaveApplicationRole_WithoutRules(1, exceptionsRoleType.Key, 2));
        };
    }
}