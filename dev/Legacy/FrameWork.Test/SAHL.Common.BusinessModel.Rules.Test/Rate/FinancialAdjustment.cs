...
namespace SAHL.Common.BusinessModel.Rules.Test.Rate
{
    ...
    public class FinancialAdjustment : RuleBase
    {
        ...
        protected IEventList<IFinancialAdjustment> falist = null;
        ...
        /// </summary>
        [NUnit.Framework.Test]
        public void FinancialAdjustmentPendingTestPass()
        {
            FinancialAdjustmentPending rule = new FinancialAdjustmentPending();

            IMortgageLoan ml = _mockery.StrictMock<IMortgageLoan>();

            financialAdjustmentType = _mockery.StrictMock<IFinancialAdjustmentType>();
            
            financialAdjustmentSource = _mockery.StrictMock<IFinancialAdjustmentSource>();

            financialAdjustment = _mockery.StrictMock<IFinancialAdjustment>();
            
            SetupResult.For(financialAdjustmentType.Key).Return((int)SAHL.Common.Globals.FinancialAdjustmentTypeSources.NonPerforming);
           
            SetupResult.For(financialAdjustmentSource.Description).Return("Non Performing");

            SetupResult.For(financialAdjustment.FinancialAdjustmentType).Return(financialAdjustmentType);

            SetupResult.For(financialAdjustment.FinancialAdjustmentSource).Return(financialAdjustmentSource);
            SetupResult.For(financialAdjustment.FromDate).Return(DateTime.Now);
            
            falist = new EventList<IFinancialAdjustment>();
            falist.Add(null,financialAdjustment);
            SetupResult.For(ml.FinancialAdjustments).Return(falist);
            SetupResult.For(ml.GetPendingFinancialAdjustmentByTypeSource(FinancialAdjustmentTypeSources.NonPerforming)).IgnoreArguments().Return(financialAdjustment);

            ExecuteRule(rule, 1, ml, FinancialAdjustmentTypeSources.NonPerforming);
        }
        [NUnit.Framework.Test]
        public void FinancialAdjustmentPendingTestFail()
        {
            FinancialAdjustmentPending rule = new FinancialAdjustmentPending();
            ...
            falist.Add(null, financialAdjustment);
            SetupResult.For(ml.FinancialAdjustments).Return(falist);
            SetupResult.For(ml.GetPendingFinancialAdjustmentByTypeSource(FinancialAdjustmentTypeSources.NonPerforming)).IgnoreArguments().Return(null);

            ExecuteRule(rule, 0, ml, FinancialAdjustmentTypeSources.NonPerforming);
        }
