...
namespace SAHL.Common.BusinessModel.Test
{
    ...
    internal delegate int ExceptionRoutine(CDVExceptions_DAO r, CDVExceptions_DAO rnext, string p_BranchNo, string p_AccountNo, int p_Remainder, int p_AccountType, string p_UnstrippedAccountNumber);

    [TestFixture]
    public class CDVRepositoryTest : TestBase
    {
        public string ErrorMessage; // populated by an error message if false is returned.
        public string ExceptionRoutine; // used to try and estimate code coverage in the test.
        private ILookupRepository repo = RepositoryFactory.GetRepository<ILookupRepository>();

        [Test]
        public void TestCDVValidation()
        {
            ICDVRepository cdvRepo = RepositoryFactory.GetRepository<ICDVRepository>();
            using (new SessionScope())
            {
                BankAccount_DAO bai = BankAccount_DAO.FindFirst();
                DateTime d1 = DateTime.Now;
                bool newOrigResult = cdvRepo.ValidateAccountNo(bai.ACBBranch.Key, bai.ACBType.Key, bai.AccountNumber);
            }
        }
        ...
        {
            int BankNumber = 0;
            IACBBranch p_Table = repo.BankBranches.ObjectDictionary[p_BranchNo];
            BankNumber = p_Table.ACBBank.Key;
            ExceptionRoutine = "";
            return PerformCDVCheck(p_AccountNo, p_BranchNo, BankNumber, p_AccType);
        }

        public bool PerformCDVCheck(String p_AccountNo, string p_BranchNo, int p_BankNumber, int p_AccType)
        {
            string HQL = "from CDV_DAO cdv where cdv.ACBBank = ? and cdv.ACBBranch = ? and cdv.ACBTypeNumber = ?";
            ...
            {
                ErrorMessage = "The account number not found in the bank file";
                return false;
            }
        ...
        private bool AcctNoValidation(ref string p_AccountNo, int p_BankNumber, string p_BranchNo, int p_AccType)
        {
            ...
            string HQL = "from AccountTypeRecognition_DAO atr where atr.ACBBank = ? and atr.ACBType = ?";
            ...
            {
                ErrorMessage = "The account number not define in AccountTypeRecognition";
                return false;
            }
            ...
            foreach (AccountTypeRecognition_DAO atr in atr_Table)
            {
                ...
                {
                    ErrorMessage = "The account length is invalid";
                    continue;
                }
                ...
                //System.Console.WriteLine("AcctNoValidation XXXXXXXXXX..." + p_AccountNo);
                return true;
            }
            return false;
        }

        public bool CDVValidation(CDV_DAO cdv, int p_AccType, string p_BranchNo, int p_BankNumber, string p_AccountNo, string p_UnstrippedAccountNumber)
        {
            ...
            else
            {
                if (cdv.ExceptionCode == null)
                {
                    ErrorMessage = "The CDV check for the account number failed";
                    return false;
                }
            ...
            {
                string HQL = "from CDVExceptions_DAO cdve where cdve.ACBBank = ? and cdve.ExceptionCode = ? and cdve.ACBType = ?";
        ...
        public int CalculateRemainder(string p_Weightings, int? p_FudgeFactor, int? p_Modulus, string p_AccountNo)
        {
