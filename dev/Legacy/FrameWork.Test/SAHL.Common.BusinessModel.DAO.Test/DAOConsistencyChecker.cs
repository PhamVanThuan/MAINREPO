...
{
    public class DAODataConsistencyChecker
    {
            ...
            Dictionary<string, object> Props, ref object Key, ActiveRecordModel arm, PrimaryKeyModel pkm, ref PrimaryKeyType pkType)
        {
            ...
            else if (pkm.PrimaryKeyAtt.Generator == PrimaryKeyType.Foreign)
            {
                ...
                foreach (OneToOneModel oto in arm.OneToOnes)
                {
                    ...
                    ActiveRecordAttribute ara = (ActiveRecordAttribute)((from attribute in FKType.GetCustomAttributes(typeof(ActiveRecordAttribute), false)
                                                                         select attribute).FirstOrDefault());
                    object obj = null;
                    if (obj == null)
                    {
                        if (!string.IsNullOrEmpty(ara.DiscriminatorColumn))
                        {
                            FKType = (from type in Assembly.Load("SAHL.Common.BusinessModel.DAO").GetTypes()
                                      where type.Name.Contains("_DAO")
                                      where type.IsSubclassOf(FKType)
                                      orderby type.Name
            ...
            #endregion Property Type Switch

            #region Belongs to

            SetBelongsTo(daoObject, KeyNameProperty);

            #endregion Belongs to
        ...
        public static void LoadSaveLoad(object daoObject, string DefaultKeyColName, bool delete)
        {
            ...
            foreach (PropertyModel pm in arm.Properties)
            {
                ...
                if (Props.ContainsKey(PropName))
                {
                    ...
                    if (ToCompareTo != null && !val.Equals(ToCompareTo))
                    {
            ...
            #endregion Load it back up and verify the values are correct
        ...
        /// <param name="daoObject"></param>
        /// <param name="DefaultKeyColName"></param>
        public static object FindFirst(object daoObject, string DefaultKeyColName)
        {
            object obj = null;
            Type ARBase = null;
            ARBase = GetARBase(daoObject);
        ...
        private static void Switch(object daoObject, Dictionary<string, object> Props, PropertyModel pm)
        {
            PropertyInfo pi = pm.Property;
            ...
            try
            {
                switch (TypeName)
                {
                    ...
                    case "Nullable`1[Int32]":
                        {
                            if (Props.ContainsKey(PropName)) break;
                            Props.Add(PropName, null);
                            pi.SetValue(daoObject, null, null);
                            break;
                        };
        ...
        private static ActiveRecordModel GetModel(Type DAOType)
        {
        ...
        private static PrimaryKeyModel GetPKModel(ActiveRecordModel arm, Type DAOObject)
        {
        ...
        private static void SetProperties(object daoObject, ActiveRecordModel arm, Dictionary<string, object> Props)
        {
        ...
        private static void SetBelongsTo(object daoObject, string KeyNameProperty)
        {
            // loop through all the object types until you find one that is generic - this is because the
            // ActiveRecordModel only gives model info for the properties of the declaring class
            // and ignores the inherited properties.
        ...
        private static Type GetARBaseForType(Type tDAO)
        {
        ...
        private static Type GetARBase(object daoObject)
        {
        ...
        public static object GetFindFirst(Type daoType)
        {
            return daoType.GetMethod("FindFirst").Invoke(null, null);
        }
        private static Type GetTypeAtLevel(Type t, int objLevels)
        {
            Type tmpType = t;
            while (objLevels > 0)
            {
        ...
        private static int GetActiveRecordBaseDepth(Type t)
        {
        ...
        private static int Get2AM_DBBaseDepth(Type t)
        {
        ...
        private static int GetObjectDepth(Type t)
        {
            Type Base = t.BaseType;
            int objLevels = 0;
            while (Base != typeof(ActiveRecordBase))
        ...
        private static Int64 GetRandomInt64()
        {
            Random r = new Random();
            return (Int64)r.Next(1, Int32.MaxValue);
        }

        private static Int16 GetRandomInt16()
        {
            Random r = new Random();
        ...
        #endregion RandomType Generators
