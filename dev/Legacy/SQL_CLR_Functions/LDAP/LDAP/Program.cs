    ...
    class Program
    {
        ...
        public class DataAccess
        {
            public static void ClearData()
            {
                ...
                sb.AppendLine("delete from LDAP_ADUser;");
                sb.AppendLine("delete from LDAP_Group;");
                sb.AppendLine("delete from LDAPUserGroup;");
                DBMan.ExecuteNonQuery(sb.ToString());
            }
            public static void UpdateUsers(LDAP_DATA ds)
            {
                UpdateInfo info = new UpdateInfo("insert into LDAP_ADUser values (@ID, @Name); ", null, null);
                //SqlParameter prm = null;
                info.InsertParams.Add(new SqlParameter("@ID", SqlDbType.Int, 50, "ID"));
                info.InsertParams.Add(new SqlParameter("@Name", SqlDbType.VarChar, 50, "Name"));
                DBMan.Update(ds.LDAP_ADUser, info);
            }
            public static void UpdateGroups(LDAP_DATA ds)
            {
                UpdateInfo info = new UpdateInfo("insert into LDAP_Group values (@ID, @Name); ", null, null);
                //SqlParameter prm = null;
                info.InsertParams.Add(new SqlParameter("@ID", SqlDbType.Int, 50, "ID"));
                info.InsertParams.Add(new SqlParameter("@Name", SqlDbType.VarChar, 50, "Name"));
                DBMan.Update(ds.LDAP_Group, info);
            }
            public static void UpdateUserGroups(LDAP_DATA ds)
            {
                UpdateInfo info = new UpdateInfo("insert into LDAPUserGroup values (@LDAP_ADUserID, @LDAP_GroupID); ", null, null);
                //SqlParameter prm = null;
                info.InsertParams.Add(new SqlParameter("@LDAP_ADUserID", SqlDbType.Int, 50, "LDAP_ADUserID"));
                info.InsertParams.Add(new SqlParameter("@LDAP_GroupID", SqlDbType.Int, 50, "LDAP_GroupID"));
                DBMan.Update(ds.LDAPUserGroup, info);
            }

            public static void GetUserGroups(DataTable dt, string UserName)
            {
                StringBuilder sb = new StringBuilder();
                sb.AppendLine("select a.Name, c.Name ");
                sb.AppendLine("from LDAP_ADUser a ");
                sb.AppendLine("join LDAPUserGroup b on a.ID=b.LDAP_ADUserID ");
                sb.AppendLine("join LDAP_Group c on b.LDAP_GroupID = c.id ");
                sb.AppendFormat("where a.Name='{0}'", UserName);
        ...
        static void Main(string[] args)
        {
            try
            {
                ldap.LDAPQueryService SVC = new LDAP.ldap.LDAPQueryService();
                string[] ss= SVC.GetFeatureAccessForUser("paulc");
                Console.WriteLine(ss.Length);
                //DataAccess.ClearData();
                //LDAP_DATA ds = new LDAP_DATA();
                //ADLookup ad = new ADLookup("SAHL");
                //List<string> Groups = ad.ListGroupsForUser("paulc");
                //DataTable dtUSers = ad.UsersInDomain();
                //List<string> Groups = ad.GroupsInDomain();
                //DataRow[] arr = new DataRow[dtUSers.Rows.Count];
                //dtUSers.Rows.CopyTo(arr, 0);

                //for (int i = 0; i < arr.Length; i++)
                //{
                //    string Name = arr[i][0].ToString();
                //    if (!Name.StartsWith("#"))
                //    {
                //        LDAP_DATA.LDAP_ADUserRow row = ds.LDAP_ADUser.NewLDAP_ADUserRow();
                //        row.ID = i;
                //        row.Name = Name;
                //        ds.LDAP_ADUser.AddLDAP_ADUserRow(row);
                //    }
                //}
                //for (int i = 0; i < Groups.Count; i++)
                //{
                //    LDAP_DATA.LDAP_GroupRow row = ds.LDAP_Group.NewLDAP_GroupRow();
                //    row.ID = i;
                //    row.Name = Groups[i];
                //    ds.LDAP_Group.AddLDAP_GroupRow(row);
                //}

                //DataAccess.UpdateUsers(ds);
                //DataAccess.UpdateGroups(ds);

                //foreach (LDAP_DATA.LDAP_ADUserRow row in ds.LDAP_ADUser.Rows)
                //{
                //    int UID = row.ID;
                //    string Name = row.Name;
                //    List<string> UserGroups = ad.ListGroupsForUser(Name);
                //    foreach (string group in UserGroups)
                //    {
                //        DataRow[] ar = ds.LDAP_Group.Select(string.Format("Name='{0}'", group));
                //        if (null != ar && ar.Length > 0)
                //        {
                //            int GroupID = Convert.ToInt32(ar[0][0]);
                //            LDAP_DATA.LDAPUserGroupRow newrow = ds.LDAPUserGroup.NewLDAPUserGroupRow();
                //            newrow.LDAP_GroupID = GroupID;
                //            newrow.LDAP_ADUserID = UID;
                //            ds.LDAPUserGroup.AddLDAPUserGroupRow(newrow);
                //        }
                //    }
                //}
                //DataAccess.UpdateUserGroups(ds);
