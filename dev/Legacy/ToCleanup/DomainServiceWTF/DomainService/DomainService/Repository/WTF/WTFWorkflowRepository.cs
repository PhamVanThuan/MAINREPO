...
namespace DomainService.Repository.WTF
{
    ...
    public class WorkflowRepository : IDisposable
    {
        ...
        public void SaveApplicationRole(ApplicationRole_WTF_DAO role)
        {
            try
            {
                role.SaveAndFlush();
...
//                {
//                    // Update
//                    sb.AppendFormat("update [TestDB_WTF]..offerrole set LegalEntityKey={0}, ", role.LegalEntityKey);
...
//                {
//                    // Insert
//                    sb.AppendFormat("insert into [TestDB_WTF]..offerRole values (");
//                    sb.AppendFormat("{0}, {1}, {2}, {3}, getdate())", role.LegalEntityKey,
//                        role.ApplicationKey, role.ApplicationRoleTypeKey, role.GeneralStatus.Key);
//                }

//                Helpers.ExecuteNonQueryOnCastleTran(sb.ToString(), typeof(GeneralStatus_WTF_DAO));
...
//                     VALUES(getdate(), 'DomainService', 'd101523', '1', 'sahl\bla', 'form', 'OfferRole', 
//                            'OfferRoleKey', '" + role.Key + "', '<XML>')";
//                Helpers.ExecuteNonQueryOnCastleTran(query, typeof(GeneralStatus_WTF_DAO));
            }
            catch (Exception ex)
            {
        ...
        public void MarkRoleAsInactive(ApplicationRole_WTF_DAO role)
        {
            role.GeneralStatus = WorkflowRepository.GetGeneralStatus_WTF_DAOByName("Inactive");// RepositoryFactory.GetRepository<ILookupRepository>().GeneralStatuses.ObjectDictionary["2"];
        ...
        public void CreateAndSaveApplicationRole_WTF_DAO(int ApplicationKey, string ORT, string ADUserName)
        {
            ApplicationRole_WTF_DAO role = new ApplicationRole_WTF_DAO();
            role.ApplicationKey = ApplicationKey;
            role.ApplicationRoleTypeKey = GetOfferRoleTypeByName_WTF(ORT).Key;
            role.GeneralStatus = GetGeneralStatus_WTF_DAOByName("Active");
        ...
        public void CreateAndSaveApplicationRole_WTF_DAO(int ApplicationKey, string ORT, int LEKey)
        {
            ApplicationRole_WTF_DAO role = new ApplicationRole_WTF_DAO();
            role.ApplicationKey = ApplicationKey;
            role.ApplicationRoleTypeKey = GetOfferRoleTypeByName_WTF(ORT).Key;
        ...
        /// <returns></returns>
        internal static ApplicationRoleType_WTF_DAO GetOfferRoleTypeByName_WTF(string Name)
        {
            ApplicationRoleType_WTF_DAO[] arr = ApplicationRoleType_WTF_DAO.FindAllByProperty("Description", Name);

            //string strHQL = "from ApplicationRoleType_WTF_DAO art where art.Description = ?";
            //SimpleQuery<ApplicationRoleType_WTF_DAO> q = new SimpleQuery<ApplicationRoleType_WTF_DAO>(strHQL, Name);
            //q.SetQueryRange(1);
            //arr = q.Execute();
        ...
        /// <returns></returns>
        internal static GeneralStatus_WTF_DAO GetGeneralStatus_WTF_DAOByName(string Name)
        {
            GeneralStatus_WTF_DAO[] arr = GeneralStatus_WTF_DAO.FindAllByProperty("Description", Name);
            if (null == arr || arr.Length == 0)
        ...
        /// <param name="Name"></param>
        /// <returns></returns>
        internal static ADUser_WTF_DAO GetADUSerByName(string Name)
        {
            ADUser_WTF_DAO[] arr = ADUser_WTF_DAO.FindAllByProperty("ADUserName", Name);
            if (null == arr || arr.Length == 0)
        ...
        {
            return GetLatestApplicationRoleByApplicationKeyAndRoleTypeKey(ApplicationKey, GetOfferRoleTypeByName_WTF(OfferRoleTypeName).Key);
        }
        /// <summary>
        ...
        {
            string HQL = "select from ApplicationRole_DAO o where o.ApplicationKey = ? and o.ApplicationRoleType.Key=? and o.GeneralStatus.Key=1 order by o.Key desc";
        ...
        {
            string HQL = "select from ApplicationRole_DAO o where o.ApplicationKey = ? and o.ApplicationRoleType.Key=? order by o.Key desc";
            SimpleQuery<ApplicationRole_DAO> q = new SimpleQuery<ApplicationRole_DAO>(HQL, ApplicationKey, ApplicationRoleTypeKey);
            q.SetQueryRange(5);
            ApplicationRole_DAO[] res = q.Execute();
            if (null == res || res.Length == 0)
        ...
        internal static ApplicationRole_DAO GetLastApplicationRoleByApplicationRoleTypeKeyAndLEKeys(int OfferRoleTypeKey, List<int> LEKeys)
        {
            ...
            sb.Remove(0, 1);
            string HQL = string.Format("select from ApplicationRole_DAO o where o.ApplicationRoleType.Key=? and o.GeneralStatus.Key=1 and o.LegalEntityKey in ({0}) order by o.Key desc", sb.ToString());
        ...
        internal static ApplicationRole_DAO FindLastApplicationRoleByApplicationRoleTypeKeyAndLEKeys(int OfferRoleTypeKey, List<int> LEKeys)
        {
            ...
            sb.Remove(0, 1);
            //string HQL = string.Format("select from ApplicationRole_DAO o where o.ApplicationRoleType.Key=? and o.GeneralStatus.Key=1 and o.LegalEntityKey in ({0}) order by o.Key desc", sb.ToString());
            string HQL = string.Format("select from ApplicationRole_DAO o where o.ApplicationRoleType.Key=? and o.LegalEntityKey in ({0}) order by o.Key desc", sb.ToString());
            SimpleQuery<ApplicationRole_DAO> q = new SimpleQuery<ApplicationRole_DAO>(HQL, OfferRoleTypeKey);
            q.SetQueryRange(5);
            ApplicationRole_DAO[] res = q.Execute();
            if (res.Length == 0)
        ...
        {
            ApplicationRoleType_WTF_DAO art = null;
            switch (orgStructure)
            {
                case OrganisationStructureGroup.Consultant:
                    {
                        art = GetOfferRoleTypeByName_WTF(string.Format("{0} Consultant D", Prefix));
                        break;
                    }
                case OrganisationStructureGroup.Admin:
                    {
                        art = GetOfferRoleTypeByName_WTF(string.Format("{0} Admin D", Prefix));
                        break;
                    }
                case OrganisationStructureGroup.Manager:
                    {
                        art = GetOfferRoleTypeByName_WTF(string.Format("{0} Manager D", Prefix));
                        break;
                    }
                default:
                    {
