...
{
    public class ResubmitToCredit : SAHLCommonBasePresenter<IWorkFlowConfirm>
    {
        CBOMenuNode _node;
        int _applicationKey = 0;

        public ResubmitToCredit(IWorkFlowConfirm view, SAHLCommonBaseController controller)
        ...
        void _view_OnYesButtonClicked(object sender, EventArgs e)
        {
            ...
            if (_applicationKey > 0)
            {
                ...
                try
                {
                    ret = appRepo.ResubmitToCredit(_applicationKey);
                    if (ret)
                        txn.VoteCommit();
                }
                catch (Exception)
