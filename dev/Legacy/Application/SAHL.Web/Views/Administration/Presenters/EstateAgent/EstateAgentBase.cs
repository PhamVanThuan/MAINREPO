...
namespace SAHL.Web.Views.Administration.Presenters.EstateAgent
{
	...
	/// EstateAgent Base
	/// </summary>
    public class EstateAgentBase : SAHLCommonBasePresenter<IExternalOrganisationStructure>
	{
        ...
        public EstateAgentBase(IExternalOrganisationStructure view, SAHLCommonBaseController controller)
        ...
        protected override void OnViewInitialised(object sender, EventArgs e)
        {
            ...
            _view.TreeViewHeading = "Estate Agencies / Agents";
        ...
        {
            //get OS for EA Channel
            IControl ct = CTRepo.GetControlByDescription("EstateAgentChannelRoot");
        ...
        void TreeNodeDragged(object sender, TreeListNodeDragEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    // Add exclusion set
                    this.ExclusionSets.Add(RuleExclusionSets.EstateAgentDragDrop);

                    // current node details
                    IEstateAgentOrganisationNode currentEstateAgentOrganisationNode = EstateAgentRepo.GetEstateAgentOrganisationNodeForKey(currentOrgStructureKey);
                    ILegalEntity currentLegalEntity = LegalEntityRepo.GetLegalEntityByKey(currentLegalEntityKey);
                    // target node details                    
                    IEstateAgentOrganisationNode targetEstateAgentOrganisationNode = EstateAgentRepo.GetEstateAgentOrganisationNodeForKey(targetOrgStructureKey);

                    // do the move
                    IEstateAgentOrganisationNode estateAgentOrganisationNode = currentEstateAgentOrganisationNode.MoveMe(targetEstateAgentOrganisationNode, currentLegalEntity);

                    //save the new org structure
                    EstateAgentRepo.SaveEstateAgentOrganisationStructure(estateAgentOrganisationNode);

                    //// if we have any validation errors lets handle them
                    //if (!_view.IsValid)
                    //    throw new Exception();

                    this.ExclusionSets.Remove(RuleExclusionSets.EstateAgentDragDrop);
        ...
        private IEstateAgentRepository _estateAgentRepo;

        public IEstateAgentRepository EstateAgentRepo
        {
            get
            {
                if (_estateAgentRepo == null)
                    _estateAgentRepo = RepositoryFactory.GetRepository<IEstateAgentRepository>();

                return _estateAgentRepo;
            }
        }
