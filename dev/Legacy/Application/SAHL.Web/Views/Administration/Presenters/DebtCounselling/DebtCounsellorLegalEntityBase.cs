...
{
    /// <summary>
    /// DebtCounsellorLegalEntity Base
    /// </summary>
    public class DebtCounsellorLegalEntityBase : SAHLCommonBasePresenter<IExternalOrganisationStructureLegalEntity>
    {
        #region Constants
        ...
        public DebtCounsellorLegalEntityBase(IExternalOrganisationStructureLegalEntity view, SAHLCommonBaseController controller)
        ...
        protected override void OnViewInitialised(object sender, EventArgs e)
        {
            ...
            _view.DisableNCRDCRegNumber = false;
			_view.DisableCompanyIntroductionDate = true;
			_view.DisableCompanyTradingName = true;
			_view.DisableCompanyRegistrationNumber = true;
			_view.DisableCompanyStatus = true;

			_view.DisableContactHomePhone = true;
			_view.DisableContactCellPhone = true;

			if (SelectedParentOrgNode != null)
			{
				_view.ParentOrgstructureTypeLabelText = "Parent " + SelectedParentOrgNode.OrganisationType.Description + " : ";
				_view.ParentOrgstructureDescLabelText = SelectedParentOrgNode.Description;
			}
        }
        ...
        protected override void OnViewPreRender(object sender, EventArgs e)
        {
            ...
            if (_view.OrganisationType != null && _view.OrganisationType.Key == (int)SAHL.Common.Globals.OrganisationTypes.Department)
                _view.AddressHidden = true;
        } 
        ...
        private void SetUpView()
        {
            ...
            switch (_view.SelectedLegalEntityType)
            {
                case (int)LegalEntityTypes.NaturalPerson:
                    _view.PanelNaturalPersonAddVisible = true;
                    _view.PanelCompanyAddVisible = false;

                    _view.DisableContactHomePhone = false;
                    _view.DisableContactCellPhone = false;
                    break;
                case (int)LegalEntityTypes.Company:
                    _view.PanelNaturalPersonAddVisible = false;
                    _view.PanelCompanyAddVisible = true;

                    if (_view.OrganisationType == null || _view.OrganisationType.Key == (int)SAHL.Common.Globals.OrganisationTypes.Company)
                        _view.DisableCompanyRegistrationNumber = false;
                    _view.DisableContactHomePhone = true;
                    _view.DisableContactCellPhone = true;
                    break;
                default:
        ...
        private void FilterLEType(List<string> filterList)
        {
            ...
            foreach (KeyValuePair<string, string> leType in leDict)
            {
				...
				{
					if (Convert.ToInt32(leType.Key) == (int)SAHL.Common.Globals.LegalEntityTypes.Company
					 || Convert.ToInt32(leType.Key) == (int)SAHL.Common.Globals.LegalEntityTypes.NaturalPerson)
					{
		...
		protected void _view_OnOrganisationTypeSelectedIndexChanged(object sender, KeyChangedEventArgs e)
		{
			...
			{
				if (_view.OrganisationType.Key == (int)SAHL.Common.Globals.OrganisationTypes.Branch_Originator)
					_view.CompanyNameLabelText = "Branch Name";
				else
					_view.CompanyNameLabelText = _view.OrganisationType.Description + " Name";
			}

			SetAddressCaptureEnabled();
		}

		protected void SetAddressCaptureEnabled()
		{
			if (_view.OrganisationType != null)
			{
        ...
        public IDictionary<string, string> LENPRoleTypes
        {
            get
            {
                if (_lenpRoleTypes == null)
                {
					...
					_lenpRoleTypes.Add(SAHL.Common.Constants.DebtCounsellor.Contact, SAHL.Common.Constants.DebtCounsellor.Contact);
                }
        ...
        public IDictionary<string, string> OrganisationTypes
        {
            get
            {
                if (_organisationTypes == null)
                {
                    ...
                    //complete lazy hack as per PDA's
                    foreach (IOrganisationType ot in LRepo.OrganisationTypes)
                    {
                        if (ot.Key == (int)SAHL.Common.Globals.OrganisationTypes.Company
                            ...
                            || ot.Key == (int)SAHL.Common.Globals.OrganisationTypes.Department)
                        {
                            if (ot.Key == (int)SAHL.Common.Globals.OrganisationTypes.Branch_Originator)
                                _organisationTypes.Add(ot.Key.ToString(), "Branch");
                            else
        ...
        private IDebtCounsellorOrganisationNode _osSel;
		public IDebtCounsellorOrganisationNode SelectedOrgNode
        {
            get
            {
                if (_osKeySet && _osKey > 0)
                {
                    if (_osSel == null)
                        _osSel = (IDebtCounsellorOrganisationNode)DCRepo.GetDebtCounsellorOrganisationNodeForKey(_osKey);
        ...
        private IDebtCounsellorOrganisationNode _osParent;
		public IDebtCounsellorOrganisationNode SelectedParentOrgNode
        {
            get
            {
                ...
                {
                    if (_osParent == null)
                        _osParent = (IDebtCounsellorOrganisationNode)DCRepo.GetDebtCounsellorOrganisationNodeForKey(_osKeyParent);
        ...
        protected List<ICacheObjectLifeTime> LifeTimes
        {
            get
            {
                if (_lifeTimes == null)
                {
                    ...
                    views.Add("AdminDebtCounsellorLegalEntityAdd");
					views.Add("AdminDebtCounsellorLegalEntityUpdate");
					views.Add("AdminDebtCounsellorLegalEntityView");
					views.Add("AdminDebtCounsellorLegalEntityRemove");
