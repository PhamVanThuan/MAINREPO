...
{
    public class LegalEntityDeathNotify : LegalEntityNotification
    {
        /// <summary>
		/// Constructor for LegalEntityDeathNotify
		/// </summary>
		/// <param name="view"></param>
		/// <param name="controller"></param>
        public LegalEntityDeathNotify(ILegalEntityNotification view, SAHLCommonBaseController controller)
			: base(view, controller)
		{
            _view.OnSubmitButtonClicked += new EventHandler(_view_OnSubmitButtonClicked);
            _view.UpdateDeath = true;
		}
        ...
        protected void _view_OnSubmitButtonClicked(object sender, EventArgs e)
        {
            using (TransactionScope txn = new TransactionScope())
            {
                ...
                {
                    //bool workdone = false;
                    Dictionary<int, bool> selectedGridItems = _view.GetDeathItems;
                    foreach (BindableLEReasonRole ler in leList)
                    {
                        foreach (KeyValuePair<int, bool> kv in selectedGridItems)
                        {
                            ...
                            {
                                if (kv.Value && !ler.Death)//checked in the grid, no reason exists, create one
                                {
                                    IReason r = ReasonRepo.CreateEmptyReason();
                                    r.Comment = "";
                                    r.GenericKey = ler.LEKey;
                                    r.ReasonDefinition = ReasonRepo.GetReasonDefinitionsByReasonDescriptionKey(ReasonTypes.DebtCounsellingNotification, (int)ReasonDescriptions.NotificationofDeath)[0];
                                    ReasonRepo.SaveReason(r);
                                    //workdone = true;
                                }

                                if (!kv.Value && ler.Death)//not selected in the grid, delete the reason (if it exists)
                                {
                                    ...
                                    {
                                        if (r.ReasonDefinition.ReasonDescription.Key == (int)ReasonDescriptions.NotificationofDeath)
                                        {
                    ...
                    if (_view.IsValid && !String.IsNullOrEmpty(affectedLE))
                    {
                        ...
                        if (leLeft)
                            template = CRepo.GetCorrespondenceTemplateByKey(CorrespondenceTemplates.DeceasedNotificationLivingExists);
                        else
                            template = CRepo.GetCorrespondenceTemplateByKey(CorrespondenceTemplates.DeceasedNotificationNoLiving);
