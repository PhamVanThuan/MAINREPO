...
namespace SAHL.Web.Views.Common.Presenters
{
    ...
    /// 
    /// </summary>
    public class CallSummaryRoute : CallSummary
    {
        #region Private members

		private IList<IADUser> _consultants;
		...
		public CallSummaryRoute(ICallSummary view, SAHLCommonBaseController controller)
        ...
        protected override void OnViewInitialised(object sender, EventArgs e)
        {
            ...
            if (_view.IsPostBack == false)
            {
                ...
                _view.SetupControlsForAddOrUpdate(true, true);
        ...
        protected override void OnViewPreRender(object sender, EventArgs e)
        {
            ...
            if (_view.Messages.Count > 0)
            {
                ...
                if (PrivateCacheData.ContainsKey("HelpDeskQueryListSelIdx"))
                {
                    ...
                    _view.SubmitButtonEnabled = false;
                    _view.AddOrUpdateButtonEnabled = true;
                    _view.SetupControlsForAddOrUpdate(false, true);
                }
                else
                    _view.SetupControlsForAddOrUpdate(true, true);
        ...
        private void BindConsultants()
        {
            ...
            _consultants = orgRepo.GetAllHelpdeskConsultants("Help Desk", secRepo.GetADUserByPrincipal(_view.CurrentPrincipal));
            _view.BindConsultantDropDown(_consultants);
        }
        ...
        void _view_OnAddUpdateClicked(object sender, EventArgs e)
        {
            ...
            {
				_update = true;
				helpDeskQuery = _helpDeskQueryList[Convert.ToInt32(this.PrivateCacheData["HelpDeskQueryListSelIdx"])];
            }
        ...
        void _view_OnSubmitClicked(object sender, EventArgs e)
        {
			...
			if (!DoRoutingValidate())
				return;

			for (int consultantIndex = 0; consultantIndex < _consultants.Count; consultantIndex++)
			{
				if (_consultants[consultantIndex].Key == _view.SelectedConsultant)
				{
					consultant = _consultants[consultantIndex];
					break;
				}
			}
			...
			{
				_helpDeskQueryList[i].Memo.ADUser = consultant;
				TransactionScope txn = new TransactionScope();
				using (txn)
				{
					try
				...
				if (_view.Messages.ErrorMessages.Count == 0 && helpDeskQueryKey != 0)
				{
					...
					Inputs.Add("CurrentConsultant", consultant.ADUserName);
					...
					x2svc.StartActivity(_view.CurrentPrincipal, _instanceID, SAHL.Common.Constants.WorkFlowActivityName.HelpDeskRoute, null, false);
		...
		protected bool DoRoutingValidate()
		{
			SAHLPrincipalCache spc = SAHLPrincipalCache.GetPrincipalCache(_view.CurrentPrincipal);
			IDomainMessageCollection dmc = spc.DomainMessages;
			bool passed = true;

			if (_view.SelectedConsultant == -1)
			{
				string errorMessage = "Please select a Consultant.";
