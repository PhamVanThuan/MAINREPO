...
{
    public class when_told_to_get_affordability_assessments_for_application : WithFakes
    {
        ...
        Establish context = () =>
        {
            ...
            autoMocker.Get<IApplicationDomainServiceClient>().WhenToldTo(x => x.PerformQuery(Param.IsAny<GetAffordabilityAssessmentsForApplicationQuery>())).Return(new SystemMessageCollection());

            autoMocker.Get<IApplicationDomainServiceClient>().WhenToldTo<IApplicationDomainServiceClient>(x => x.PerformQuery(Param.IsAny<GetAffordabilityAssessmentsForApplicationQuery>())).Callback<GetAffordabilityAssessmentsForApplicationQuery>(y => { y.Result = serviceQueryResult; });
        };

        Because of = () =>
        {
            result = autoMocker.ClassUnderTest.GetAffordabilityAssessmentsForApplication(Param.IsAny<int>());
        };
