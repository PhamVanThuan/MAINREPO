...
namespace SAHL.Common.BusinessModel.Specs.Rules.Valuations.CheckActiveValuation12Months
{
    ...
    public class when_there_is_an_inactive_valuation_within_12months : RulesBaseWithFakes<SAHL.Common.BusinessModel.Rules.Valuations.CheckActiveValuation12Months>
    {
        ...
        Establish context = () =>
        {
            ...
            valuation1.WhenToldTo(x => x.ValuationDate).Return(DateTime.Now.AddMonths(-11));

            IValuation valuation2 = An<IValuation>();
            valuation2.WhenToldTo(x => x.IsActive).Return(true);
            valuation2.WhenToldTo(x => x.ValuationDate).Return(DateTime.Now.AddMonths(-13));
