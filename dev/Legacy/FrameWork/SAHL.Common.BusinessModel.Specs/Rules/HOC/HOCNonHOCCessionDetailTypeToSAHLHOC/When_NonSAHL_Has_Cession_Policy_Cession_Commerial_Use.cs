...
{
    internal class When_NonSAHL_Has_Cession_Policy_Cession_Commerial_Use : RulesBaseWithFakes<SAHL.Common.BusinessModel.Rules.HOC.HOCNonHOCCessionDetailTypeToSAHLHOC>
    {
        ...
        Establish context = () =>
        {
            ...
            IDetail detail2 = An<IDetail>();
            IDetailType detailType = An<IDetailType>();
            IDetailType detailType2 = An<IDetailType>();

            detailType.WhenToldTo(x => x.Key).Return((int)DetailTypes.HOCCessionOfPolicy);
            detailType2.WhenToldTo(x => x.Key).Return((int)DetailTypes.HOCCessionCommercialUse);

            detail.WhenToldTo(x => x.DetailType).Return(detailType);
            detail2.WhenToldTo(x => x.DetailType).Return(detailType2);
            IEventList<IDetail> details = new EventList<IDetail>(new IDetail[]{ detail,detail2});
