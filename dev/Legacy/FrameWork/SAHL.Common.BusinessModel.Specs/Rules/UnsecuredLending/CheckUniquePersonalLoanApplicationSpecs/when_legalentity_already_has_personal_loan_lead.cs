...
namespace SAHL.Common.BusinessModel.Specs.Rules.UnsecuredLending.CheckUniquePersonalLoanApplicationSpecs
{
	...
	public class when_legalentity_already_has_personal_loan_lead : RulesBaseWithFakes<CheckUniquePersonalLoanApplication>
	{
		Establish context = () =>
		{
			var castleTransactionsService = An<SAHL.Common.BusinessModel.Interfaces.Service.ICastleTransactionsService>();
            castleTransactionsService.WhenToldTo(x => x.Many<SAHL.Common.BusinessModel.Interfaces.IExternalRole>(Param.IsAny<Globals.QueryLanguages>(), Param.IsAny<string>(), Param.IsAny<string>(), Param.IsAny<Globals.Databases>(), Param.IsAny<object[]>())).Return(new List<SAHL.Common.BusinessModel.Interfaces.IExternalRole>() { An<IExternalRole>() });
