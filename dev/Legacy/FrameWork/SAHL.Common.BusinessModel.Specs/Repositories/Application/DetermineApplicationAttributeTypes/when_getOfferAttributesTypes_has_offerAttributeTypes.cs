...
{
	[Subject(typeof(when_getOfferAttributesTypes_has_offerAttributeTypes))]
	internal class when_getOfferAttributesTypes_has_offerAttributeTypes : WithFakes
	{
		...
		Establish context = () =>
		{
			...
			applicationAttributesFromSource.Columns.Add("Remove", typeof(bool));
			applicationAttributesFromSource.Rows.Add("1", "false");

			dataSet = new DataSet();
			dataSet.Tables.Add(applicationAttributesFromSource.Clone());
			dataSet.Tables[0].Rows.Add("1", "false");
		...
		It should_return_keys_from_source = () =>
		{
