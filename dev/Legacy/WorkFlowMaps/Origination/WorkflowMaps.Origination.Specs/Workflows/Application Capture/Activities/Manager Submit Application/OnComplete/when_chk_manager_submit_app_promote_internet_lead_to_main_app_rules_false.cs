...
namespace WorkflowMaps.Origination.Specs.Workflows.Application_Capture.Activities.Manager_Submit_Application.OnComplete
{
    ...
    internal class when_chk_manager_submit_app_promote_internet_lead_to_main_app_rules_false : WorkflowSpecApplicationCapture
    {
        ...
        private Establish context = () =>
        {
            ...
            client.WhenToldTo(x => x.CheckManagerSubmitApplicationRules(Param.IsAny<IDomainMessageCollection>(), Param.IsAny<int>(), Param.IsAny<bool>()))
                .Return(true);
            client.WhenToldTo(x => x.PromoteLeadToMainApplicant(Param.IsAny<IDomainMessageCollection>(), Param.IsAny<int>(), Param.IsAny<bool>()))
                .Return(false);
