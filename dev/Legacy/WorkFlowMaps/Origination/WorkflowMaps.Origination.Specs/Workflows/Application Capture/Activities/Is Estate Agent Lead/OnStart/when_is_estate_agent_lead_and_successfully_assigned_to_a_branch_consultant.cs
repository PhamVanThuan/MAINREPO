...
namespace WorkflowMaps.Origination.Specs.Workflows.Application_Capture.Activities.Is_Estate_Agent_Lead.OnStart
{
    ...
    internal class when_is_estate_agent_lead_and_successfully_assigned_to_a_branch_consultant : WorkflowSpecApplicationCapture
    {
        ...
        private static int consultantCallCount;
        private static int adminCallCount;
        private static int managerCallCount;

        private Establish context = () =>
        {
            result = false;
            consultantCallCount = 0;
            adminCallCount = 0;
            managerCallCount = 0;
            ...
            assignment.Expect(x => x.AssignCreateorAsDynamicRole((IDomainMessageCollection)messages, instanceData.ID, "Branch Consultant D", out assignedTo, workflowData.ApplicationKey, "Estate Agent Lead Assign"))
                ...
                .Return(false)
                .WhenCalled((y) => { consultantCallCount++; });
            assignment.Expect(x => x.AssignCreateorAsDynamicRole((IDomainMessageCollection)messages, instanceData.ID, "Branch Admin D", out assignedTo, workflowData.ApplicationKey, "Estate Agent Lead Assign"))
                .OutRef(string.Empty)
                .Return(false)
                .WhenCalled((y) => { adminCallCount++; });
            assignment.Expect(x => x.AssignCreateorAsDynamicRole((IDomainMessageCollection)messages, instanceData.ID, "Branch Admin D", out assignedTo, workflowData.ApplicationKey, "Estate Agent Lead Assign"))
                .OutRef(string.Empty)
                .Return(false)
                .WhenCalled((y) => { managerCallCount++; });
        ...
        private It should_assign_creator_as_branch_consultant = () =>
        {
            consultantCallCount.ShouldEqual(1);
        };

        private It should_not_assign_creator_as_branch_admin = () =>
        {
            adminCallCount.ShouldEqual(0);
        };

        private It should_not_assign_creator_as_branch_manager = () =>
        {
            managerCallCount.ShouldEqual(0);
        };
