...
namespace WorkflowMaps.Origination.Specs.Workflows.Application_Management.States.Disputes.OnEnter
{
    ...
    internal class when_disputes_and_is_fl : WorkflowSpecApplicationManagement
    {
        ...
        {
            client.WasToldTo(x => x.ReactiveUserOrRoundRobinForOSKeyByProcess((IDomainMessageCollection)messages, "FL Processor D", workflowData.ApplicationKey, 157, instanceData.ID, "Disputes",
        ...
        private It should_not_deactivate_origination_users = () =>
        {
            client.WasNotToldTo(x => x.DeActiveUsersForInstanceAndProcess((IDomainMessageCollection)messages, instanceData.ID, workflowData.ApplicationKey, workflowRoles, SAHL.Common.Globals.Process.Origination));
        };

        private It should_not_reactivate_branch_users_for_origination = () =>
        {
            client.WasNotToldTo(x => x.ReActivateBranchUsersForOrigination((IDomainMessageCollection)messages, instanceData.ID, workflowData.AppCapIID, workflowData.ApplicationKey, "Disputes",
