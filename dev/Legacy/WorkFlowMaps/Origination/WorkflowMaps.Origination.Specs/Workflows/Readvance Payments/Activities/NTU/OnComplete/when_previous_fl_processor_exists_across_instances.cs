...
namespace WorkflowMaps.Origination.Specs.Workflows.Readvance_Payments.Activities.NTU.OnComplete
{
    ...
    internal class when_previous_fl_processor_exists_across_instances : WorkflowSpecReadvancePayments
    {
        ...
        private It should_not_round_robin_assign_a_fl_processor = () =>
        {
            wfa.WasNotToldTo(x => x.X2RoundRobinForPointerDescription((IDomainMessageCollection)messages, instanceData.ID, (int)SAHL.Common.Globals.RoundRobinPointers.FLProcessor,
                workflowData.ApplicationKey, "FL Processor D", "NTU", SAHL.Common.Globals.Process.Origination));
        };

        private It should_reactivate_the_existing_processor = () =>
        {
            wfa.WasToldTo(x => x.ReassignCaseToUser(Param.IsAny<IDomainMessageCollection>(), Param.IsAny<long>(), Param.IsAny<int>(),
