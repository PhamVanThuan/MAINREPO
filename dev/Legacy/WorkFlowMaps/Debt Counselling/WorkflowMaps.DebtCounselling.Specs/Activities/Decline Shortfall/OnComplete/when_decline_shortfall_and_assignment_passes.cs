...
namespace WorkflowMaps.DebtCounselling.Specs.Activities.Decline_Shortfall.OnComplete
{
    [Subject("Activity => Decline_Shortfall => OnComplete")]
    internal class when_decline_shortfall_and_assignment_passes : WorkflowSpecDebtCounselling
    {
        ...
        private Because of = () =>
        {
            result = workflow.OnCompleteActivity_Decline_Shortfall(instanceData, workflowData, paramsData, messages, ref message);
        };
        ...
        private It should_assign_or_load_balance_to_the_debt_counselling_consultant = () =>
        {
            wfa.WasToldTo(x => x.AssignDebtCounsellingCaseForGroupOrLoadBalance((IDomainMessageCollection)messages, instanceData.ID, workflowData.DebtCounsellingKey,
                SAHL.Common.Globals.WorkflowRoleTypes.DebtCounsellingConsultantD, "Pend Cancellation", exclusionStates, false, workflowData.CourtCase));
        };
    }
}