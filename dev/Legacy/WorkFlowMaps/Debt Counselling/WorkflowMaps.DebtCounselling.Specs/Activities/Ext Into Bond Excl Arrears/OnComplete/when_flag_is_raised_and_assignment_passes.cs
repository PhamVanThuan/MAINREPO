...
namespace WorkflowMaps.DebtCounselling.Specs.Activities.Ext_Into_Bond_Excl_Arrears.OnComplete
{
    [Subject("Activity => Ext_Into_Bond_Excl_Arrears => OnComplete")]
    internal class when_flag_is_raised_and_assignment_passes : WorkflowSpecDebtCounselling
    {
        ...
        private Because of = () =>
        {
            result = workflow.OnCompleteActivity_Ext_Into_Bond_Excl_Arrears(instanceData, workflowData, paramsData, messages, ref message);
        };
        ...
        private It should_assign_or_load_balance_to_the_debt_counselling_consultant = () =>
        {
            wfa.WasToldTo(x => x.AssignDebtCounsellingCaseForGroupOrLoadBalance((IDomainMessageCollection)messages, instanceData.ID, workflowData.DebtCounsellingKey,
                SAHL.Common.Globals.WorkflowRoleTypes.DebtCounsellingConsultantD, "Bond Exclusions Arrears", exclusionStates, false, workflowData.CourtCase));
        };
    }
}