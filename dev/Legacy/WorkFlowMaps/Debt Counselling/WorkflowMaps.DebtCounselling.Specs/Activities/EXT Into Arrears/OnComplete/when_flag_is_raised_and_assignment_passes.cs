...
namespace WorkflowMaps.DebtCounselling.Specs.Activities.EXT_Into_Arrears.OnComplete
{
    [Subject("Activity => EXT_Into_Arrears => OnComplete")]
    internal class when_flag_is_raised_and_assignment_passes : WorkflowSpecDebtCounselling
    {
        ...
        private Because of = () =>
        {
            result = workflow.OnCompleteActivity_EXT_Into_Arrears(instanceData, workflowData, paramsData, messages, ref message);
        };
        ...
        private It should_assign_or_load_balance_to_the_debt_counselling_consultant = () =>
        {
            wfa.WasToldTo(x => x.AssignDebtCounsellingCaseForGroupOrLoadBalance((IDomainMessageCollection)messages, instanceData.ID, workflowData.DebtCounsellingKey,
                SAHL.Common.Globals.WorkflowRoleTypes.DebtCounsellingConsultantD, "Default in Payment", exclusionStates, false, workflowData.CourtCase));
        };
    }
}