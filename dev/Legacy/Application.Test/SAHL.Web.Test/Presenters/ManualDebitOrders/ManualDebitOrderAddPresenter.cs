...
namespace SAHL.Web.Test.Presenters.ManualDebitOrders
{
    [Ignore]
    [TestFixture]
    public class ManualDebitOrderAddPresenter : TestViewBase
    {
        ...
        public void InitialiseTest()
        {
            ...
            _mockery.ReplayAll();
            ManualDebitOrderAdd presenter = new ManualDebitOrderAdd(_view, controller);
            presenter.Node = _node;
...
//            SetupResult.For(AR.GetAccountByFinancialServiceKey( -1)).IgnoreArguments().Return(account);
            ...
            Expect.Call(FSR.GetFinancialServiceByKey( -1)).IgnoreArguments().Return(fs);

            _recurringTransactions = _mockery.CreateMock<IEventList<IFinancialServiceRecurringTransaction>>();

            SAHL.Common.BusinessModel.Interfaces.IAccount account2 = _mockery.CreateMock<SAHL.Common.BusinessModel.Interfaces.IAccount>();

            SetupResult.For(fs.Account).Return(account2);

            IReadOnlyEventList<IFinancialServiceRecurringTransaction> recurringTransactions = _mockery.CreateMock<IReadOnlyEventList<IFinancialServiceRecurringTransaction>>();

            IReadOnlyEventList<ILegalEntity> lstLegalEntities = _mockery.CreateMock<IReadOnlyEventList<ILegalEntity>>();

            int[] roleTypes = new int[2] { 2, 3 };
            Expect.Call(account2.GetLegalEntitiesByRoleType(null, roleTypes)).IgnoreArguments().Return(lstLegalEntities);

            SetupResult.For(lstLegalEntities.Count).Return(1);

            SetupResult.For(fs.FinancialServiceRecurringTransactions).Return(_recurringTransactions);
