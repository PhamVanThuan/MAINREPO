...
IF EXISTS (select * from dbo.sysobjects where id = object_id(N'test.InsertNewTestDC') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
Begin
	Drop Procedure test.InsertNewTestDC
	Print 'Dropped procedure test.InsertNewTestDC'
End
Go

CREATE PROCEDURE test.InsertNewTestDC

@DebtCounsellorName varchar(250),
@regNumber varchar(250),
@NCRNumber varchar(250)
...
values
(3,getdate(), @regNumber, @DebtCounsellorName, '011', '22323232', 'test@test.co.za',1,1,'SAHL\TestUser',2)
...
--debtCounsellorDetail
insert into debtCounselling.debtCounsellorDetail
select @LEKey, @NCRNumber
