...
IF EXISTS
	(SELECT * FROM dbo.SysObjects WHERE ID=Object_ID(N'[test].[InsertCSknockOut]') AND ObjectProperty(ID,N'IsProcedure')=1)
BEGIN
	DROP PROCEDURE [test].[InsertCSknockOut]
	PRINT 'DROPPED PROCEDURE [test].[InsertCSknockOut]'
END
GO
	
/***********************************************************************************************************************
PROC INSERTS AN ITC V4 RECORD FOR THE INDIVIDUAL APPLICANT THAT RESULTS IN THE KNOCKOUT POLICY INDICATORS BEING FLAGGED.
...
KNOCKOUT PARAMETERS:
@knockOut		(Decline 'DebtReview' and Refer 'Disputes')
@G012			(Decline 0 to 7)
@G032			(Decline 0 to 11)
@G044			(Decline 1 or greater)
@WEPPstatus		(Decline 4 to 9 or E, H, I, J, L, W, AA, AC) (Refer 'No payment profile')
@EmpiricaExclusionCode (D = Deceased, N = Insufficient Data, L = Legal, Z = Empirica is not available)
...
CREATE PROCEDURE [test].[InsertCSknockOut]
	(
		@offerKey int,
		@idNumber varchar(13),
...
DECLARE @knockOutXml varchar(max),
		@accountKey int,
		@legalEntityKey int,
		@firstNames varchar(50),
		@surname varchar(50),
		@xml varchar(max),
		@exclusionCodeXml varchar(max)
...
--by default set the exclusion code to be normal
SET @exclusionCodeXml = '<ExclusionCode />'
--if we have given a code then we need to update the empirica score and the exclusion code
IF (@EmpiricaExclusionCode <> 'N/A')
	BEGIN
		SET @EmpiricaScore = '000'	
		SET @exclusionCodeXml = '<ExclusionCode>'+@EmpiricaExclusionCode+'</ExclusionCode>'
	END
...
IF @knockOut='DebtReview'
	BEGIN
		SET @knockOutXml='<DebtCounsellingDC01 xmlns="https://secure.transunion.co.za/TUBureau">
    <ConsumerNo>577041784</ConsumerNo>
    <DebtCounsellingDate>20070717</DebtCounsellingDate>
    <DebtCounsellingCode>DAP</DebtCounsellingCode>
    <DebtCounsellingDescription>Application for Debt Review</DebtCounsellingDescription>
  </DebtCounsellingDC01>'
	END

IF @knockOut='Disputes'
	BEGIN
		SET @knockOutXml='<DisputeIndicatorDI01 xmlns="https://secure.transunion.co.za/TUBureau">
    <ConsumerNo>710998981</ConsumerNo>
    <CaptureDate>20070517</CaptureDate>
    <ExpiryDate>20070613</ExpiryDate>
    <DaysToExpiry>07</DaysToExpiry>
    <Surname>'+@surname+'</Surname>
    <Forename1>'+@firstNames+'</Forename1>
    <Forename2 />
    <Forename3 />
    <DateOfBirth>/01/19</DateOfBirth>
    <IdentityNo>'+@idNumber+'</IdentityNo>
  </DisputeIndicatorDI01>'
	END
...
SELECT @xml='<BureauResponse xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  ...
  <StandardBatchCharsSB33 xmlns="https://secure.transunion.co.za/TUBureau">
    ...
    <RE105>'+@RE105+'</RE105>
    <RE140>'+@RE140+'</RE140>
    <WEPP_Status>'+@WEPPstatus+'</WEPP_Status>
