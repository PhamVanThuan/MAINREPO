...
namespace SAHL.Services.Capitec.Specs.CommandHandlerSpecs.ApplyForSwitchLoanCommandHandlerSpecs
{
    public class when_applying_for_a_loan_given_a_successful_application : WithFakes
    {
        ...
        private Establish context = () =>
        {
            ...
            applicationService.WhenToldTo(x => x.RecalculateSwitchApplication(Param.IsAny<LoanApplication>(), Param.IsAny<Dictionary<Guid, Applicant>>(), true, applicationID)).Return(true);
        ...
        {
            applicationService.WasToldTo<IApplicationManager>(x => x.PerformITCs(Param.IsAny<Dictionary<Guid, Applicant>>()));
        };
        ...
        private It should_recalculate_the_application = () =>
        {
            applicationService.WasToldTo(x => x.RecalculateSwitchApplication(
                ...
                Param<Dictionary<Guid, Applicant>>.Matches(m => m.Values.Contains(applicants[0])), true, applicationID));
        };
        ...
        {
            serviceCommandRouter.WasToldTo(x => x.HandleCommand(Param<CreateSAHomeLoansSwitchApplicationCommand>.Matches(a => a.ApplicationType == Interfaces.Capitec.Common.Enumerations.ApplicationStatusEnums.InProgress && a.SystemMessageCollection.WarningMessages().Count() == 0), metadata));
        };
    }
}