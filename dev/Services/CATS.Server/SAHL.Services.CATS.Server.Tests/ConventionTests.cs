    ...
    public class ConventionTests : ConventionTestSuite
    {
        ...
        public void CheckForRuleExecutionHandlers()
        {
            ...
            foreach (var test in testParams)
            {
                this.Run(MethodBase.GetCurrentMethod().Name, test.TypeUnderTest.Name, test, () =>
                {
                    AssemblyDefinition commandHandlerDefinition = AssemblyDefinition.ReadAssembly(test.TypeUnderTest.Assembly.Location);
                    if (commandHandlerDefinition == null || commandHandlerDefinition.Modules == null || !commandHandlerDefinition.Modules.Any())
                    {
                        return;
                    }
                    // for each command handler
                    var typeDefinition = commandHandlerDefinition.Modules
                        .SelectMany(t => t.Types)
                        .FirstOrDefault(x => x.Name == test.TypeUnderTest.Name);
                    if (typeDefinition == null)
                    {
                        return;
                    }
                    // has a constructor that takes a IDomainRuleManager<T> parameter
                    var hasConstructorThatTakesIDomainRuleManager = !typeDefinition.Methods.Where(c => c.IsConstructor)
                        .SelectMany(p => p.Parameters)
                        .Any(p => p.ParameterType.Name.Contains("IDomainRuleManager"));
                    if (hasConstructorThatTakesIDomainRuleManager)
                    {
                        return;
                    }
                    // get the Constructor
                    var constructor = typeDefinition.Methods.SingleOrDefault(c => c.IsConstructor);
                    if (constructor == null)
                    {
                        return;
                    }
                    var isRegisterRuleCalledOnConstructor = !constructor.Body.Instructions
                        .Where(x => x.OpCode == OpCodes.Callvirt)
                        .Select(x => x.Operand)
                        .OfType<MethodReference>()
                        .Any(x => x.DeclaringType.FullName.Contains("SAHL.Core.Rules.IDomainRuleManager") &&
                            x.Name == "RegisterRule" &&
                            x.ReturnType.Name == "Void");
                    if (isRegisterRuleCalledOnConstructor)
                    {
                        return;
                    }
                    var handleCommandMethod = typeDefinition.Methods.SingleOrDefault(h => h.Name == "HandleCommand");
                    if (handleCommandMethod == null)
                    {
                        return;
                    }
                    bool executeRulesIsCalled = handleCommandMethod.Body.Instructions
                        .Where(x => x.OpCode == OpCodes.Callvirt)
                        .Select(x => x.Operand)
                        .OfType<MethodReference>()
                        .Any(x => x.DeclaringType.FullName.Contains("SAHL.Core.Rules.IDomainRuleManager") &&
                    ...
                    var message = string.Format("All command handlers that register a rule should execute the rule. {0} does not meet the convention.", typeDefinition.Name);
                    Assert.True(executeRulesIsCalled, message);
                });
            }
        }
            ...
            foreach (var test in testParams)
            {
                this.Run(MethodBase.GetCurrentMethod().Name, test.TypeUnderTest.Name, test, () =>
                {
                    ...
                    foreach (var arg in commandCtorArgs)
                    {
                        ...
                        {
                            if (String.Compare(arg.Name, property.Name, true) != 0)
                            {
                                continue;
                            }
                            argMatched = true;
        ...
        public void EnsureAllPropertiesOnModelsAreMarkedWithDataContractAttribute()
        {
            ...
            foreach (var test in testParams)
            {
                ...
                {
                    var properties = test.TypeUnderTest.GetProperties();
