...
namespace SAHL.Services.CATS.Server.Specs.HandlerSpec.NotifyBatchPaymentRecipients
{
    public class when_successful : WithFakes
    {
        ...
        private It should_execute_the_rules = () =>
         {
             domainRuleManager.WasToldTo(x => x.ExecuteRules(Param.IsAny<ISystemMessageCollection>(), Param<CatsPaymentBatchRuleModel>.Matches(r => r.BatchKey == command.CATSPaymentBatchKey)));
         };
        ...
        private It should_raise_an_event = () =>
         {
             eventRaiser.WasToldTo(x => x.RaiseEvent(Param.IsAny<DateTime>(),
                     ...
                     e.SuccessfulNotifications.First().GenericKey == catsPaymentBatchItems.First().GenericKey
                     && e.SuccessfulNotifications.First().GenericTypeKey == catsPaymentBatchItems.First().GenericTypeKey
                     && e.SuccessfulNotifications.First().Amount == catsPaymentBatchItems.First().Amount
                     && e.UnSuccessfulNotifications.Count() == 0
                 )
