...
{
    public class when_creating_commands_for_user_start_with_split_request : WithFakes
    {
        ...
        private Establish context = () =>
        {
            ...
            request = new X2RequestForExistingInstance(Guid.NewGuid(), instanceId, X2RequestType.UserStart, serviceRequestMetadata, activityName, false);
        };
        ...
        private It should_get_an_activityviewmodel_for_the_requested_activity = () =>
        {
            automocker.Get<IWorkflowDataProvider>().WasToldTo(x => x.GetActivityForInstanceAndName(instanceId, activityName));
        };

        private It should_return_a_userrequeststartactivitywithsplitcommand = () =>
        {
            commands.First().ShouldBe(typeof(UserRequestStartActivityWithSplitCommand));
        };
    }
}