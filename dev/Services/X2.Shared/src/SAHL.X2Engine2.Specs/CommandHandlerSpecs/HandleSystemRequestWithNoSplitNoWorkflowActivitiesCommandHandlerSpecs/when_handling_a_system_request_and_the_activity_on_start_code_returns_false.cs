...
{
    public class when_handling_a_system_request_and_the_activity_on_start_code_returns_false : WithFakes
    {
        ...
        private Establish context = () =>
        {
            ...
            command.Result = true; // this is to ensure that we actually set the command result
            ...
            x2Map.WhenToldTo(x => x.StartActivity(Param.IsAny<InstanceDataModel>(), Param.IsAny<IX2ContextualDataProvider>(), Param.IsAny<IX2Params>(), Param.IsAny<ISystemMessageCollection>()))
                .Return(false);
            handleMapReturnCommand = new HandleMapReturnCommand(instance.ID, false, systemMessageCollection, activity.ActivityName, WorkflowMapCodeSectionEnum.OnStart);
        ...
        It should_get_the_Activity_message_for_the__next_state = () =>
        {
            x2Map.WasNotToldTo(x => x.GetActivityMessage(Param.IsAny<InstanceDataModel>(), Param.IsAny<IX2ContextualDataProvider>(), Param.IsAny<IX2Params>(), Param.IsAny<ISystemMessageCollection>()));
        };

        It should_not_allow_case_to_exit_the_current_state = () =>
        {
        ...
        It should_set_map_result_to_the_command_result = () =>
        {
            handleMapReturnCommand.Result.ShouldEqual(command.Result);
        };
    }
}