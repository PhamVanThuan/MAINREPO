...
{
    public class when_handling_a_system_request_and_the_following_states_on_enter_code_returns_false : WithFakes
    {
        ...
        private Establish context = () =>
        {
            ...
            x2Map.WhenToldTo(x => x.StartActivity(Param.IsAny<InstanceDataModel>(), Param.IsAny<IX2ContextualDataProvider>(), Param.IsAny<IX2Params>(), Param.IsAny<ISystemMessageCollection>()))
                .Return(true);
            x2Map.WhenToldTo(x => x.GetActivityMessage(Param.IsAny<InstanceDataModel>(), Param.IsAny<IX2ContextualDataProvider>(), Param.IsAny<IX2Params>(), Param.IsAny<ISystemMessageCollection>()))
            ...
            x2Map.WhenToldTo(x => x.EnterState(Param.IsAny<InstanceDataModel>(), Param.IsAny<IX2ContextualDataProvider>(), Param.IsAny<IX2Params>(), Param.IsAny<ISystemMessageCollection>()))
                .Return(false);
            handleMapReturnCommand = new HandleMapReturnCommand(instance.ID, false, systemMessageCollection, activity.ActivityName, WorkflowMapCodeSectionEnum.OnEnter);
        ...
        It should_not_write_a_stage_transition_record = () =>
        {
            autoMocker.Get<IX2ServiceCommandRouter>().WasNotToldTo<IX2ServiceCommandRouter>(
