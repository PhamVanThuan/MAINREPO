...
{
    public class when_checking_if_a_user_complete_is_a_user_request : WithFakes
    {
        private static AutoMocker<X2RequestInterrogator> autoMocker;
        private static IX2Request request;
        private static bool result = false, expectedResult = true;

        private Establish context = () =>
        {
            autoMocker = new StructureMap.AutoMocking.NSubstitute.NSubstituteAutoMocker<X2RequestInterrogator>();
            request = new X2Request(Guid.NewGuid(), X2RequestType.UserComplete, null, false, null);
        };
