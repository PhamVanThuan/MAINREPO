...
{
    public class when_asked_for_a_legacy_process_that_has_not_yet_been_loaded : WithFakes
    {
        ...
        Establish context = () =>
        {
            ...
            var proxyProcess = An<IX2LegacyProcess>();
            proxy.WhenToldTo(x => x.GetProcess(Arg.Any<string>())).Return(proxyProcess);
            ...
            processDataModel = new ProcessDataModel(processId, null, applicationName, "", new byte[] { }, DateTime.Now, "", ",", "3", false);
        ...
        It should_instantiate_a_legacy_process = () =>
        {
            process.ShouldBeOfType(typeof(X2TLSLegacyProcess));
        };
    }
}