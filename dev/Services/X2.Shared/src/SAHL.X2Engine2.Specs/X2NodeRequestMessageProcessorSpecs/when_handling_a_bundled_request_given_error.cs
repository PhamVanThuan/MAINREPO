...
{
    public class when_handling_a_bundled_request_given_error : WithFakes
    {
        ...
        {
            autoMocker.Get<IRawLogger>().WasToldTo(x => x.LogErrorWithException(Arg.Any<LogLevel>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<Exception>(),Arg.Any<IDictionary<string, object>>()));
        };

        It should_publish_error_response_to_x2_error_queue = () =>
        {
            autoMocker.Get<IX2ResponsePublisher>().WasNotToldTo(x => x.Publish(Arg.Is<IX2RouteEndpoint>(y => y.ExchangeName == X2QueueManager.X2EngineErrorExchange && y.QueueName == X2QueueManager.X2EngineErrorQueue), Arg.Any<X2Response>()));
        };
