...
namespace SAHL.Services.DecisionTreeDesign.Server.Specs.Services.CurrentlyOpenDocumentManagerSpecs
{
    ...
    public class when_opening_a_document_by_different_user : WithFakes
    {
        ...
        Establish context = () =>
        {
            ...
            currentlyOpenDocumentDataManager.WhenToldTo(x => x.IsDocumentOpenByUser(documentVersionId,username)).Return(false);
            currentlyOpenDocumentManager = new CurrentlyOpenDocumentManager(currentlyOpenDocumentDataManager);
        };

        Because of = () =>
        {
            exception = Catch.Exception(()=> currentlyOpenDocumentManager.OpenDocument(documentVersionId, documentTypeId, username));
        };
