        ...
        public void RecurseJObject(JToken jObj, string tabs)
        {
            if (jObj != null)
            {
                ...
                if (groups != null && groups.Count() > 0)
                {
                    ...
                    {
                        this.WriteLine("{0}namespace {1}", tabs, ToPascalCase(group["name"].ToString().Trim()));
                ...
                if (enumerations != null)
                {
                    ...
                    {
                        var output = value["value"].Select(x => Sanitize(x.ToString())).Aggregate((c, n) => c + "," + n);
                        this.WriteLine("{0}public enum {1} {{ {2} }}", tabs, Sanitize(value["name"].ToString()), output);
                    }
                }
            }
        }
