...
{
    public class UpdateLastAssignedUserByCapabilityForInstanceHandler : ITableProjector<WorkflowCaseAssignedEvent, IDataModel>
    {
        private readonly IDbFactory dbFactory;

        public UpdateLastAssignedUserByCapabilityForInstanceHandler(IDbFactory dbFactory)
        {
        ...
        public void Handle(WorkflowCaseAssignedEvent @event, IServiceRequestMetadata metadata)
        {
            using (var context = this.dbFactory.NewDb().InAppContext())
            {
                ...
                var statement = new UpdateLastAssignedUserByCapabilityForInstanceStatement(@event.InstanceId,
                    @event.CapabilityKey,
