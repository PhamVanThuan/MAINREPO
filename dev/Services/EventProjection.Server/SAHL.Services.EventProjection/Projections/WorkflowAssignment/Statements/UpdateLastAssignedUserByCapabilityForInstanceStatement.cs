...
{
    public class UpdateLastAssignedUserByCapabilityForInstanceStatement : ISqlStatement<object>
    {
        ...
        public UpdateLastAssignedUserByCapabilityForInstanceStatement(long instanceId, int capabilityKey, int userOrganisationStructureKey,
            int genericKeyTypeKey, int genericKey, string userName)
        {
        ...
        {
            return @"
MERGE [EventProjection].[projection].[LastAssignedUserByCapabilityForInstance] WITH(HOLDLOCK) AS target --with holdlock ensures we perform the join and subsequent insert (if the value is missing) atomically
...
ON (target.InstanceId = source.InstanceId and target.CapabilityKey = source.CapabilityKey)
WHEN MATCHED THEN
    UPDATE SET
