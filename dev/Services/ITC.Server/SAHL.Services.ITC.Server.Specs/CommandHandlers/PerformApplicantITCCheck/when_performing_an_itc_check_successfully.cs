...
{
    public class when_performing_an_itc_check_successfully : WithFakes
    {
        ...
        private Establish context = () =>
        {
            ...
            itcResponse = new ItcResponse("",
                ...
                XDocument.Parse("<request></request>"),
                XDocument.Parse("<response></response>"));
            itcRequest = new ItcRequest();
            itcResponse = new ItcResponse("",
                "",
                DateTime.Now,
        ...
        private It should_not_log_itc_request_or_response = () =>
        {
            itcManager.WasNotToldTo(
                x => x.LogFailedITCRequestAndResponse(Param.IsAny<ItcRequest>(), Param.IsAny<ItcResponse>(), Param.IsAny<string>()));
        };
