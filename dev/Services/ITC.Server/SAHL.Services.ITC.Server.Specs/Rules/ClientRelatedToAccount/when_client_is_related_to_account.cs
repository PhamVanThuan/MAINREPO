...
{
    class when_client_is_related_to_account : WithFakes
    {
        ...
        private Establish context = () =>
        {
            ...
            applicationDomainClient.WhenToldTo(x => x.PerformQuery(Param<DoesAccountBelongToClientQuery>.Matches(y => y.IdNumber == command.IdNumber)))
                .Return<DoesAccountBelongToClientQuery>(y =>
                {
                    y.Result = new ServiceQueryResult<DoesAccountBelongToClientQueryResult>(
                    new List<DoesAccountBelongToClientQueryResult> { new DoesAccountBelongToClientQueryResult(){ OfferKey = 0} });
