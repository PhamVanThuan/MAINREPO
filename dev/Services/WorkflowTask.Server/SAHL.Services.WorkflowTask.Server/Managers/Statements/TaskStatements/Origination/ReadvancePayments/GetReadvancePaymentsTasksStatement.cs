...
namespace SAHL.Services.WorkflowTask.Server.Managers.Statements.TaskStatements.Origination.ReadvancePayments
{
    internal class GetReadvancePaymentsTasksStatement : TaskBaseStatement
    {
        public GetReadvancePaymentsTasksStatement(string username, List<string> roles, bool includeTasksWithTheRoleOfEveryone,
...
SELECT WL.InstanceID
    ...
    ,WL.Subject
    ,[Application Type] = ot.Description
...
FROM (
    ...
    FROM (
        ...
        INNER JOIN [X2].[X2DATA].Readvance_Payments X2D ON X2D.InstanceID = I.ID
...
INNER JOIN [2AM]..OfferType ot ON ot.OfferTypeKey = WL.OfferTypeKey
INNER JOIN [2AM]..OfferInformationVariableLoan oivl ON oivl.OfferInformationKey = WL.OfferInformationKey
