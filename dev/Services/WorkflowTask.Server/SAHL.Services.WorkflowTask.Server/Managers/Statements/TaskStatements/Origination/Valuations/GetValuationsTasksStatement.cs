...
namespace SAHL.Services.WorkflowTask.Server.Managers.Statements.TaskStatements.Origination.Valuations
{
    internal class GetValuationsTasksStatement : TaskBaseStatement
    {
        public GetValuationsTasksStatement(string username, List<string> roles, bool includeTasksWithTheRoleOfEveryone, string businessProcessName,
...
FROM (
    ...
    FROM (
        ...
        INNER JOIN [X2].[X2DATA].Valuations X2D WITH (NOLOCK) ON X2D.InstanceID = I.ID
