...
{
    public class SwitchApplicationDomainProcess : ApplicationCreationDomainProcess<SwitchApplicationCreationModel>
    {
        public SwitchApplicationDomainProcess(
            ...
            combGuidGenerator, clientDataManager, x2WorkflowManager, linkedKeyManager, propertyDomainService, communicationManager, applicationDataManager, domainRuleManager, rawLogger, loggerSource, loggerAppSource)
        {
        }
        ...
        public override void OnInternalStart()
        {
            if (ValidateApplication(applicationStateMachine, domainRuleManager))
            {
                ...
                domainModelMapper.CreateMap<SwitchApplicationCreationModel, SwitchApplicationModel>();
                var switchApplicationCreationModel = domainModelMapper.Map(this.DataModel);

                var addSwitchBasicApplicationCommand = new AddSwitchApplicationCommand(switchApplicationCreationModel, serviceRequestMetadata.CommandCorrelationId);
                var serviceMessages = applicationDomainService.PerformCommand(addSwitchBasicApplicationCommand, serviceRequestMetadata);
