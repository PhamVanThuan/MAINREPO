...
{
    public class RefinanceApplicationDomainProcess : ApplicationCreationDomainProcess<RefinanceApplicationCreationModel>
    {
        public RefinanceApplicationDomainProcess(
            ...
            : base(applicationStateMachine, applicationDomainService, clientDomainService, addressDomainService, financialDomainService,
        ...
        public override void OnInternalStart()
        {
            if (ValidateApplication(applicationStateMachine, domainRuleManager))
            {
                ...
                domainModelMapper.CreateMap<RefinanceApplicationCreationModel, RefinanceApplicationModel>();
                var RefinanceApplicationCreationModel = domainModelMapper.Map(this.DataModel);

                var addRefinanceBasicApplicationCommand = new AddRefinanceApplicationCommand(RefinanceApplicationCreationModel, serviceRequestMetadata.CommandCorrelationId);
                var serviceMessages = applicationDomainService.PerformCommand(addRefinanceBasicApplicationCommand, serviceRequestMetadata);
