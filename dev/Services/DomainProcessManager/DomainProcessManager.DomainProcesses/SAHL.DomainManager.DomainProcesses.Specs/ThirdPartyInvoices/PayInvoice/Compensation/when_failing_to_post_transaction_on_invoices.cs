...
{
    public class when_failing_to_post_transaction_on_invoices : PayThirdPartyInvoiceProcessSpec
    {
        ...
        private static IEnumerable<PayThirdPartyInvoiceModel> nonPostingInvoices;
        ...
        private Establish context = () =>
        {
            ...
            nonPostingInvoices = dataModel.InvoiceCollection.Where(y => y.ThirdPartyInvoiceKey == 18378 || y.ThirdPartyInvoiceKey == 18379);

            stuckInvoiceQueue = new ConcurrentQueue<int>();
            Parallel.ForEach(nonPostingInvoices, (invoice) =>
            {
                invoice.StepInProcess = PaymentProcessStep.PostingTransactionFailed;
        ...
        private It should_remove_failed_invoices_from_payment_batch = () =>
        {
            financeDomainService.Received(nonPostingInvoices.Count()).PerformCommand(
               Arg.Is<RemoveThirdPartyInvoiceFromPaymentBatchCommand>(m =>
                   m.CATSPaymentBatchKey == batchReference &&
                  nonPostingInvoices.Select(i => i.ThirdPartyInvoiceKey).Contains(m.ThirdPartyInvoiceKey)
               )
        ...
        {
            x2WorkflowManager.Received(nonPostingInvoices.Count()).ReversePayment(
