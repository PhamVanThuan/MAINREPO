...
{
    public class when_prev_file_passed : WithFakes
    {
       ...
       // private static GetPreviousFileFailureQuery getPreviousFileFailureQuery;
        ...
        private Establish context = () =>
        {
            ...
            catsService.WhenToldTo(x => x.PerformQuery(Param<GetPreviousFileFailureQuery>
                ...
                .Return<GetPreviousFileFailureQuery>((getPreviousFileFailureQueryResult) => {getPreviousFileFailureQueryResult.Result = 
                    new ServiceQueryResult<GetPreviousFileFailureQueryResult>
                        (new List<GetPreviousFileFailureQueryResult>{});
        ...
        private It should_get_confirm_file_passed = () =>
        {
            catsService.WasToldTo(x => x.PerformQuery(Param<GetPreviousFileFailureQuery>
                .Matches(y => y.CATSPaymentBatchType == Core.BusinessModel.Enums.CATSPaymentBatchType.ThirdPartyInvoice)));
        };

        private It should_not_return_an_previous_file_failed_error_message = () =>
        {
            systemMessages.ErrorMessages().Any(x => x.Message.Equals(errorMessage)).ShouldBeFalse();
        };
    }
}