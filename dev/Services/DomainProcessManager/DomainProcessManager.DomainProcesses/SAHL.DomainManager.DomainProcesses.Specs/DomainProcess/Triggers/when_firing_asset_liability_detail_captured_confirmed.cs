...
{
    public class when_firing_asset_liability_detail_captured_confirmed : WithFakes
    {
        ...
        private Because of = () =>
        {
            applicationStateMachine.FireStateMachineTrigger(ApplicationStateTransitionTrigger.AssetLiabilityDetailCapturedConfirmed, Guid.NewGuid());
        };

        private It should_transition_to_asset_liability_detail_captured_state = () =>
        {
            applicationStateMachine.Machine.IsInState(ApplicationState.AssetLiabilityDetailCaptured).ShouldBeTrue();
        };
    }
}