...
{
    public class when_firing_application_mailing_address_capture_confirmed : WithFakes
    {
        ...
        private Because of = () =>
        {
            applicationStateMachine.FireStateMachineTrigger(ApplicationStateTransitionTrigger.ApplicationMailingAddressCaptureConfirmed, Guid.NewGuid());
        };

        private It should_transition_to_application_mailing_address_captured_state = () =>
        {
            applicationStateMachine.Machine.IsInState(ApplicationState.ApplicationMailingAddressCaptured).ShouldBeTrue();
        };
    }
}