...
{
    public class when_firing_application_funding_confirmed : WithFakes
    {
        ...
        private It should_transition_state_to_application_funded_state = () =>
        {
            applicationStateMachine.ContainsStateInBreadCrumb(ApplicationState.ApplicationFunded).ShouldBeTrue();
        };

        private It should_auto_transition_state_to_minimum_application_created = () =>
        {
            applicationStateMachine.IsInState(ApplicationState.ValidMinimumApplicationCreated).ShouldBeTrue();
        };
    }
}
