...
{
    public class when_adding_a_new_purchase_and_all_addresses_fail : WithDomainServiceMocks
    {
        ...
        protected Establish context = () =>
        {
            ...
            bankAccountLinkedToClientEvent = DomainProcessTestHelper.GetBankAccountLinkedToClientEvent(bankAccountKey,
                clientKey, clientBankAccountKey, bankAccount, testDate);

            applicationDebitOrder = newPurchaseCreationModel.ApplicationDebitOrder;
            debitOrderAddedToApplicationEvent = DomainProcessTestHelper.GetDebitOrderAddedToApplicationEvent(applicationNumber, clientBankAccountKey, applicationDebitOrder, testDate);
            ...
            applicantAffordabilitiesAddedEvent = DomainProcessTestHelper.GetApplicantAffordabilitiesAddedEvent(clientKey, applicationNumber,
                applicant.Affordabilities.ToList(), testDate);

            applicantDeclarationsAddedEvent = DomainProcessTestHelper.GetApplicantDeclarationsAddedEvent(clientKey, applicationNumber, applicant.ApplicantDeclarations, testDate);
        };
        ...
        private Because of = () =>
        {
            ...
            metadata = new DomainProcessServiceRequestMetadata(domainProcess.DomainProcessId, GetGuidForCommandType(typeof(LinkBankAccountToClientCommand)));
            domainProcess.HandleEvent(bankAccountLinkedToClientEvent, metadata);

            metadata = new DomainProcessServiceRequestMetadata(domainProcess.DomainProcessId, GetGuidForCommandType(typeof(AddApplicationDebitOrderCommand)));
            domainProcess.HandleEvent(debitOrderAddedToApplicationEvent, metadata);

            metadata = new DomainProcessServiceRequestMetadata(domainProcess.DomainProcessId, GetGuidForCommandType(typeof(LinkExternalVendorToApplicationCommand)));
            domainProcess.HandleEvent(externalVendorLinkedEvent, metadata);

            metadata = new DomainProcessServiceRequestMetadata(domainProcess.DomainProcessId, GetGuidForCommandType(typeof(AddComcorpOfferPropertyDetailsCommand)));
            domainProcess.HandleEvent(comcorpPropertyDetailAddedEvent, metadata);

            metadata = new DomainProcessServiceRequestMetadata(domainProcess.DomainProcessId, GetGuidForCommandType(typeof(AddApplicantAffordabilitiesCommand)));
            domainProcess.HandleEvent(applicantAffordabilitiesAddedEvent, metadata);

            metadata = new DomainProcessServiceRequestMetadata(domainProcess.DomainProcessId, GetGuidForCommandType(typeof(AddApplicantDeclarationsCommand)));
            domainProcess.HandleEvent(applicantDeclarationsAddedEvent, metadata);
        };

        private It should_not_add_a_postal_address = () =>
        {
            addressDomainService.WasNotToldTo(x => x.PerformCommand(Param.IsAny<LinkFreeTextAddressAsPostalAddressToClientCommand>(),
                Param.IsAny<DomainProcessServiceRequestMetadata>()));
        };

        private It should_not_add_a_residential_address = () =>
        {
            addressDomainService.WasNotToldTo(x => x.PerformCommand(Param.IsAny<LinkFreeTextAddressAsResidentialAddressToClientCommand>(),
                Param.IsAny<DomainProcessServiceRequestMetadata>()));
        };
