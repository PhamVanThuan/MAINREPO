...
{
    public class when_successfully_paying_invoice : WithFakes
    {
        ...
        private Because of = () =>
        {
            x2workflowManger.ProcessThirdPartyInvoicePayment(instanceId, accountKey, thirdPartyInvoiceKey, serviceRequestMetadata);
        };

        private It should_start_pay_activity_for_the_given_case = () =>
        {
            x2Service.WasToldTo(x => x.PerformCommand(Param<X2RequestForExistingInstance>.Matches(m =>
                    m.ActivityName == X2Constants.WorkFlowActivities.ThirdPartyInvoicesProcessPayment &&
        ...
        private It should_complete_pay_activity_for_the_given_case = () =>
        {
            x2Service.WasToldTo(x => x.PerformCommand(Param<X2RequestForExistingInstance>.Matches(m =>
                   m.ActivityName == X2Constants.WorkFlowActivities.ThirdPartyInvoicesProcessPayment &&
                   m.CorrelationId == performPayActivityCorrelationId &&
