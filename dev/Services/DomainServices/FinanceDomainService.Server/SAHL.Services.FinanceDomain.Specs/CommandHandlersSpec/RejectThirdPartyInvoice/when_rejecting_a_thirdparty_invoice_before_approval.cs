...
{
    public class when_rejecting_a_thirdparty_invoice_before_approval : WithCoreFakes
    {
        ...
        {
            invoice = new ThirdPartyInvoiceDataModel(2, "ref", 1, 12345, Guid.NewGuid(), "098876", DateTime.Now, "jane@doe.com", 10, 1, 12, true, DateTime.Now, string.Empty);
            ...
            thirdPartyInvoiceDataManager.WhenToldTo(x => x.HasThirdPartyInvoiceBeenApproved(command.ThirdPartyInvoiceKey)).Return(false);
        ...
        private It should_raise_a_third_party_invoice_rejected_pre_approval_event = () =>
        {
            eventRaiser.WasToldTo(x => x.RaiseEvent(Param.IsAny<DateTime>(),
                    Param<ThirdPartyInvoiceRejectedPreApprovalEvent>.Matches(y => y.AccountNumber == invoice.AccountKey &&
