...
{
    public class when_checking_approval_by_role_with_a_mandate_higher_than_invoice_amount : WithFakes
    {
        private static IThirdPartyInvoiceApprovalMandateProvider thirdPartyInvoiceApprovalMandateProvider;
        private static DivisionalManagerCannotApproveInvoicesGreaterThanR60000Rule thirdPartyInvoiceApprovalRoleAmountRule;
        ...
        {
            approverRole = "Divisional Manager";
            thirdPartyInvoiceApprovalMandateProvider = An<IThirdPartyInvoiceApprovalMandateProvider>();
            thirdPartyInvoiceApprovalRoleAmountRule = new DivisionalManagerCannotApproveInvoicesGreaterThanR60000Rule(thirdPartyInvoiceApprovalMandateProvider);
            systemMessages = SystemMessageCollection.Empty();

            invoiceLineItems = new List<InvoiceLineItemModel> { new InvoiceLineItemModel(null,1212, 1, 3212.34M, true),
            ...
            thirdPartyInvoiceApprovalMandateProvider.WhenToldTo(y => y.GetMandatedRange(approverRole))
                .Return(new Tuple<decimal, decimal>(45000.01M, 60000.00M));
        };
