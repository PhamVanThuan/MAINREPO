...
{
    public class when_address_does_not_exist : WithCoreFakes
    {
        ...
        private Establish context = () =>
        {
            ...
            handler = new AddFixedPropertyAssetToClientCommandHandler(domainRuleManager, domainQueryService, assetLiabilityDataManager
            ...
            domainRuleManager.WhenToldTo(x => x.ExecuteRules(Param.IsAny<ISystemMessageCollection>(), Param.IsAny<FixedPropertyAssetModel>()))
                .Callback<ISystemMessageCollection, FixedPropertyAssetModel>((y, z) => { y.AddMessages(expectedMessages.AllMessages); });

            assetLiabilityDataManager.WhenToldTo(x => x.CheckIsAddressLinkedToClientAsFixedPropertyAsset
                (Param.IsAny<int>(), Param.IsAny<int>())).Return(false);
            ...
            var isAddressAClientAddressQuery = new ServiceQueryResult<IsAddressAClientAddressQueryResult>(
                ...
                {
                    new IsAddressAClientAddressQueryResult() { AddressIsAClientAddress = false }
                });
        ...
        {
            domainQueryService.WasToldTo
                (x => x.PerformQuery(Param.IsAny<IsAddressAClientAddressQuery>()));
        };
        ...
        {
            assetLiabilityDataManager.WasNotToldTo
                (x => x.SaveFixedPropertyAsset(Param.IsAny<FixedPropertyAssetModel>()));
        };
