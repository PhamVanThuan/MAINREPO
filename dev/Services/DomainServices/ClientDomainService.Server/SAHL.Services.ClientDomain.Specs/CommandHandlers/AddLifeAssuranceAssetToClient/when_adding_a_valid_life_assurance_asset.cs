...
{
    public class when_adding_a_valid_life_assurance_asset : WithCoreFakes
    {
        ...
        Establish context = () =>
        {
            ...
            assetLiabilityDataManager.WhenToldTo(x => x.SaveLifeAssuranceAsset(Param.IsAny<LifeAssuranceAssetModel>())).Return(assetLiabilityKey);

            legalEntityAssetLiabilityKey = 3333;
            assetLiabilityDataManager
                .WhenToldTo(x => x.LinkAssetLiabilityToClient(Param.IsAny<int>(), Param.IsAny<int>())).Return(legalEntityAssetLiabilityKey);
        ...
        private It should_add_life_assurance_asset = () =>
        {
            assetLiabilityDataManager.WasToldTo
                (x => x.SaveLifeAssuranceAsset(Arg.Is<LifeAssuranceAssetModel>(y => y.CompanyName == lifeAssuranceAsset.CompanyName)));
        };
        ...
        private It should_raise_a_life_assurance_asset_added_event = () =>
        {
