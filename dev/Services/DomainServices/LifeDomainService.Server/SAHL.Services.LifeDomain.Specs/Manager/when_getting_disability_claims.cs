...
{
    public class when_getting_disability_claims : WithCoreFakes
    {
        ...
        private Because of = () =>
        {
            disabilityClaims = lifeDomainDataManager.GetDisabilityClaimsByAccount(Param<int>.IsAnything);
        };

        private It should_use_the_sql_statement = () =>
        {
            dbFactory.FakedDb.InReadOnlyAppContext().WasToldTo(x => x.Select(Param<GetDisabilityClaimsByAccountStatement>.IsAnything));
        };
    }
}