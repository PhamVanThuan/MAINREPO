...
{
    public class when_approving_a_disability_claim_with_a_rule_exception : WithCoreFakes
    {
        ...
        private Establish context = () =>
        {
            ...
            domainRuleManager.WhenToldTo(x => x.ExecuteRules(Param.IsAny<ISystemMessageCollection>(), Param.IsAny<IDisabilityClaimApproveModel>()))
                .Callback<ISystemMessageCollection, IDisabilityClaimApproveModel>((y, z) => { y.AddMessages(expectedMessages.AllMessages); });
        };
        ...
        private It should_not_update_the_status_of_the_disability_claim_to_approved = () =>
        {
            lifeDomainDataManager.WasNotToldTo(x => x.UpdateDisabilityClaimStatus(disabilityClaimKey, DisabilityClaimStatus.Approved));
        };

        private It should_not_update_the_disability_claim_payment_dates = () =>
        {
            lifeDomainDataManager.WasNotToldTo(x => x.UpdateDisabilityClaimPaymentDates(disabilityClaimKey, paymentStartDate, numberOfInstalmentsAuthorised, paymentEndDate));
        };

        private It should_not_call_the_halo_api_to_produce_the_payment_schedule = () =>
        {
            lifeDomainDataManager.WasNotToldTo(x => x.CreateDisabilityClaimPaymentSchedule(disabilityClaimKey, adUserName));
        };
