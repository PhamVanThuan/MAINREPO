...
{
    public class when_the_applicant_has_no_surname : WithFakes
    {
        ...
        private Establish context = () =>
        {
            ...
            domainQueryClient.WhenToldTo(c => c.PerformQuery(Param.IsAny<GetClientDetailsQuery>())).Return<GetClientDetailsQuery>(y =>
            {
                y.Result = new ServiceQueryResult<GetClientDetailsQueryResult>(new GetClientDetailsQueryResult[] {
                            new GetClientDetailsQueryResult{ LegalEntityKey = 1234, FirstNames = "Clint", Surname = string.Empty, IDNumber = "8211045229080",
        ...
        {
            messages.ErrorMessages().First().Message.ShouldEqual("A natural person applicant must have a surname.");
        };
    }
}