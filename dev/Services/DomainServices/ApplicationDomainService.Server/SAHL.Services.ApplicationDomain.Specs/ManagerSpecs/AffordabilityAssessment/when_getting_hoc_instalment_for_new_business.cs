...
namespace SAHL.Services.ApplicationDomain.Specs.ManagerSpecs.AffordabilityAssessment
{
    public class when_getting_hoc_instalment_for_new_business : WithFakes
    {
        ...
        private Because of = () =>
        {
            affordabilityAssessmentManager.GetHocInstalmentForNewBusinessApplication(Param.IsAny<int>());
        };

        private It should_call_the_affordability_assessment_data_manager = () =>
        {
            affordabilityAssessmentDataManager.WasToldTo(x => x.GetHocInstalmentForNewBusinessApplication(0));
        };
    }
}