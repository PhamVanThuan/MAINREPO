'use strict';
angular.module('sahl.ui.halo.views.wizardPages.wizards.thirdParty.invoice.thirdPartyInvoiceCaptureWizardPageState.tpl.html',[])
.controller('Wizards_ThirdParty_Invoice_ThirdPartyInvoiceCaptureWizardPageStateCtrl',['$scope', '$stateParams',
   ...
   function($scope, $stateParams, $thirdPartyInvoiceManagerService,$financeDomainSharedModels, $accountsManagerService, 
    ...
    $pageFactory,$activityManager, $wizardFactory, $modalManagerService, $thirdPartyManagerService){
            ...
            function intialiseScope(){
                ...
                $scope.invoiceCalendarDate = new Date();
                ...
                $scope.captureThirdPartyInvoice = invoiceUtility.captureThirdPartyInvoice;
                $scope.cancelThirdPartyInvoiceCapture = invoiceUtility.cancelThirdPartyInvoiceCapture;
                $scope.update
            ...
            function loadData(){
                ...
                invoiceUtility.getLineItemCategoriesAndDescriptions()
                    ...
                    .then(addInitialRowAndStopActivity);
            };         
            ...
            var addInitialRowAndStopActivity = function(){                
            ...
            var gridUtility = {
              gridSettings: { 
                        ...
                        columnDefs: [
                         ...
                         {   name: 'lineItemDesc', 
                          ...
                          { name: 'lineItemAmount', displayName: 'Amount', cellFilter: 'currencyFilter', class:'uiCellTextRight', enableCellEdit: false},
                          { 
                              name: 'isVatable', 
                          ...
                          { name: 'lineItemVatAmount', displayName: 'VAT', cellFilter: 'currencyFilter', enableCellEdit: false},
                          { name: 'lineItemTotalAmtInclVAT', displayName: 'Amount Including VAT', cellFilter: 'currencyFilter', enableCellEdit: false,  class:'uiCellTextRight'},
              ...
              updateGridTotal: function(){
                      var runningTotal = 0;
                      $scope.invoiceGrid.data.forEach(function(data){
              ...
              updateGripDescriptionDropdown: function(row, cellSettings){
            ...
            var invoiceUtility = {
              getLineItemCategoriesAndDescriptions : function(){
                      var deferred = $q.defer();
                      invoiceUtility.getInvoiceLineItemCategories().then(function () {
                          var waitingFor = [];
              ...
              getInvoiceLineItemCategories: function(){
                      ...
                      $thirdPartyInvoiceManagerService.getLookupLineItemTypes().then(function(data){
                          ...
                          $scope.invoiceGrid.columnDefs[2].editDropdownOptionsArray = $scope.invoiceLineItemCategories;
              ...
              getInvoiceDataById: function(){
                      ...
                      $thirdPartyInvoiceManagerService.getInvoiceDataById(invoiceParameters.invoiceId).then(function(data){
                          ...
                          //this is strangely different to the one we get back on the amend page??
                          //Going to hunt out why this keeps changing!  Wait till i find the culprit!                          
              ...
              createThirdPartyInvoiceModel: function(){
                      ...
                      _.each($scope.invoiceGrid.data, function(element, index, list){
                          if(!$scope.lineItems){
                              lineItemId = $scope.lineItems[index].id || null;
                          }
                      ...
                      var thirdPartyInvoiceModel = null;
                      if(errorMessages.length == 0){
                          thirdPartyInvoiceModel = new $financeDomainSharedModels.ThirdPartyInvoiceModel($scope.invoice.id, 
                                                        $scope.selectedThirdParty, $scope.invoice.invoiceNumber, invoiceDate,
                                                        invoiceLineItems, $scope.invoice.capitaliseInvoice, $scope.invoice.paymentReference); 
                      }else{
              ...
              displayLineItemValidationMessages : function(errorMessagesCollection){
                  _.each(errorMessagesCollection, function(rowErrorMessageCollection, rowIndex, errorMessageList){
              ...
              getInvoiceLineItems: function(){
                      ...
                      $thirdPartyInvoiceManagerService.getInvoiceLineItems(invoiceParameters.invoiceId).then(function(data){    
                        //this awful line of code will change once Chris has adjusted the rest api to do a .count on the data obj                    
              ...
              captureThirdPartyInvoice: function(){  

                      if(!invoiceUtility.isInvoiceValid()){
                        return;
                      }

                      var model = invoiceUtility.createThirdPartyInvoiceModel();
                      if(model != null){
                          $thirdPartyInvoiceManagerService.captureThirdPartyInvoice(model)
                                        .then(invoiceUtility.events.invoiceCaptureSuccess, 
                                              invoiceUtility.events.invoiceCaptureFailure);
                      }
              },
              cancelThirdPartyInvoiceCapture: function(){
                /**finish off workflow**/
              ...
              events: {
                      invoiceCaptureSuccess: function(results){
                      ...
                      invoiceCaptureFailure: function(results){
                              $toastManagerService.error({
                                  title: 'Error',
                                  text: results.results[0].Message
                              }).promise.then(function() {
                                  //should we be cancelling the workflow action here?
                              });
                      }
              }
                
            };
            /* invoice utility ends */
