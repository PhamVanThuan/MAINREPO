...
{
    public class ActiveValuationChildTileContentDataProvider : HaloTileBaseContentDataProvider<ActiveValuationModel>
    {
        public ActiveValuationChildTileContentDataProvider(IDbFactory dbFactory)
            : base(dbFactory)
        {
        }
        ...
        public override string GetSqlStatement(BusinessContext businessContext)
        {
            return string.Format(@"SELECT
                ...
                FROM
                    [2am].[dbo].[Valuation] v
                    join [2am].[dbo].[Valuator] vv on vv.ValuatorKey=v.ValuatorKey
                ...
                WHERE
                    v.ValuationKey = {0}
                    and v.isActive = 1",  businessContext.BusinessKey.Key);
        }
    }
}