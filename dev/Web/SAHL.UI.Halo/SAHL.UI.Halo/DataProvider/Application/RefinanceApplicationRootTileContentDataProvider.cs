...
{
    public class RefinanceApplicationRootTileContentDataProvider : HaloTileBaseContentDataProvider<RefinanceApplicationRootModel>
    {
        public RefinanceApplicationRootTileContentDataProvider(IDbFactory dbFactory)
            : base(dbFactory)
        {
        }
        ...
        public override string GetSqlStatement(BusinessContext businessContext)
        {
            return string.Format(@"select o.OfferKey as ApplicationNumber, 
                       ...
                       isnull(oivl.ExistingLoan,0) as ExistingLoan, 
                       isnull(oivl.RequestedCashAmount,0) as CashOut,
                       round(oivl.FeesTotal,2) as TotalFees, 
                       round(oivl.LoanAgreementAmount,2) as LoanAgreementAmount,
                       case
                            when v.ValuationKey is null then oml.ClientEstimatePropertyValuation
                ...
                from   [2am].[dbo].Offer o 
                       left join [2am].[dbo].OfferInformation oi on oi.OfferInformationKey = (
                           select top 1 OfferInformationKey 
                                from  [2am].[dbo].OfferInformation 
                                where OfferKey=o.OfferKey order by ChangeDate desc)
                       ...
                       left join [2am].[dbo].Offerexpense cancfee on o.offerkey = cancfee.offerkey  AND cancfee.expensetypekey = 5 
